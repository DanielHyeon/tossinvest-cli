...
`git diff --quiet`, 역사를 걷는 아홉 자리, 그리고 `execution_baseline.py` 의 읽기가 있다.
...
- **새 거절이 오늘 거절하는 정상 입력은 0 이다 — 세어서 하는 말이다.** 정규 파일 아님 · 목록 실패 ·
  16 MiB 초과 · UTF-8 아님의 네 거절이 오늘 저장소의 번들 파일 **12,412** 중 거절하는 것은 없다
  (`analysis/harness/7524_census.py`). 7.5.22 가 더한 다섯째(본문 없는 `*.go`)도 0 이다 —
  `base-commit.txt` 를 가진 change **117** 전부에서 `*.go` 파일 줄 **97,235** 를 세어 `-`/`-` 가 **0**
  이었다. "0" 은 **측정**이지 수리의 성질이 아니다.
...
`--name-only` 이 아니라 `--numstat` 을 읽는다: 본문 억제는 `-`/`-`, mode-only 는 `0`/`0` 이다.
