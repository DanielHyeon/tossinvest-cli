...
**범위는 거기까지다 (2026-09-22 정정).** 자식 프로세스가 읽는 것은 원장에 **하나도** 안 남는다.
`check_analysis.py` 의 `subprocess.run` 은 **열여섯 자리**이고 원장 항목은 **0** 이다 — 앞 판본이
...
(7.5.23 정정: "아홉" 이라고 적었는데 6 + 9 가 16 이 안 된다).
판정 도중 그것들을 건드리면 재확인이 통과한다. a112 실측: `required` 를 정하는 Go 파일 **32** 중
