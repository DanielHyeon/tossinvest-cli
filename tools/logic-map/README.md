...
넘는 추적 `*.go`(깔때기의 상한 — **안 바뀐** 파일이어도 멈춘다, 저장소 최대 97 KB). 7.5.25 는 객체 디렉터리 경로에
`:`·`"` 가 든 저장소도 거절했는데 7.5.31 이 C 인용으로 적어 풀었고, 이름이 UTF-8 이 아닌 안 바뀐 파일도 이제 멈추지 않는다. `skip-worktree` 인데 파일이 없으면 sparse 로 **안 꺼낸** 것으로 읽고
...
pathspec 환경 변수(`GIT_LITERAL_PATHSPECS` · `GIT_GLOB_PATHSPECS`)는 워킹트리 바이트가 아니라 파서의 문제다 — 7.5.26.
함수 지도를 뽑는 파서(`extract_go_ast.go`)를 **워킹트리에서** 빌드하므로 파서를 고친 편집이 자기를 감춘다 — 7.5.32.
