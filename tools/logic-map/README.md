...
디스크를 읽는 자리가 깔때기 넷(파일 · 디렉터리 목록 · 트리 순회 · 종류)뿐이고 그 깔때기가 읽은 결과를 원장에
적으므로, 재확인의 집합은 **구조적으로** 판정의 집합이다(a112 한 판 실측: 경로 1,739). 실패도 적는다 —
...
(추적 Go 파일 편집) 기록을 만들지 않는다. 그 기록은 지울 수 없고, 그 편집이 든 분기는 창 밖에 남는다.
...
대한 진술이 아니다(게이트가 여는 순간에만 FIFO로 바꿨다 되돌리는 것으로 감사를 끌 수 있었다 — 실측 6/14).
건너뛰는 모양은 **사라진 파일 하나**뿐이고, 그 실패도 원장에 남아 되돌려 놓으면 재확인이 잡는다.
