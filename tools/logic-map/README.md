...
마다 디스크 바이트를 깔때기(`_read_regular`, 원장에 남아 끝의 재확인이 다시 읽는다)로 읽고 그 바이트의 blob 을 **임시**
객체 저장소에 쓴다. 판정의 현재 쪽도 디스크를 다시 읽지 않고 **같은 바이트**로 함수 지도를 뽑는다. 실제 인덱스와 객체
...
비용은 판정마다 추적 `*.go` 전수를 읽고 해시하는 것과 base 의 트리 전부를 검증해 읽는 것이다(이 저장소 트리 4,093,
`cat-file` 한 번). git 호출 수는 바뀐 파일 수와 무관해졌다 — 바뀐 것이 없으면 10, 있으면 blob 을 읽는 `cat-file` 이 하나
...
**안 닫는 것.** diff 의 **출력 형식**을 바꾸는 설정(`diff.noprefix` · `diff.mnemonicPrefix` · `color.ui=always`)은
워킹트리 바이트가 아니라 파서의 문제다 — 7.5.26(pathspec 환경 변수는 7.5.34 가 닫았다). 함수 지도를 뽑는
