...
**에이전트가 알아서 쓰게 하려면 MCP, 전체 기능·스크립트가 필요하면 CLI.** 결정적 차이는 **에이전트가 도구의 존재를 아는 법**입니다 — MCP 는 등록만 하면 툴 목록에 떠서 알아서 호출하고, CLI 는 프롬프트에 언급하거나 스킬·`AGENTS.md`/`CLAUDE.md` 로 알려줘야 씁니다(안 알려주면 설치된 줄도 모릅니다).

| 방식 | 이럴 때 | 에이전트가 인식하는 법 | 시작 |
|---|---|---|---|
| **CLI** (`tossctl …`) | 터미널·스크립트·자동화. **전체 기능**(공식+WTS), 결정적·파이프 | 프롬프트에 언급하거나 **스킬/`AGENTS.md` 로 등록** | 바로 아래 ↓ |
| **MCP** (`tossctl mcp`) | 에이전트에 상시로 물려 자연어로. **공식 API 범위** | **등록만 하면 자동 인식** (별도 안내 불필요) | [MCP 빠른 시작 →](#mcp-빠른-시작--3단계) |
...
| 커버 범위 | **전부** — 공식 API + WTS 전용 기능(인기 순위·브리핑·스크리너·수급 등) | **공식 API 범위**(조회 + 공식 주문). WTS 전용 기능은 미포함 |
| 자연어 | 에이전트가 자연어 → `tossctl` 명령으로 실행 | 에이전트가 자연어 → MCP 툴 호출 |

- **AI 에이전트는 둘 다 씁니다 — 차이는 '아는 법'입니다.** MCP 는 등록하면 툴로 자동 노출돼 별도 안내 없이 호출됩니다(공식 API 범위). CLI 는 셸을 다루는 에이전트(Claude Code·Codex·Cursor)가 잘 실행하지만, **프롬프트에 언급하거나 스킬·`AGENTS.md`/`CLAUDE.md` 로 알려줘야** 존재를 압니다 — 대신 **전체 기능**(공식+WTS)에 결정적·파이프 가능합니다.
