...
**CLI 가 더 많이 합니다** — **전체 기능**(공식 API + WTS 전용 21종), **공식 키 없이도** 동작, 결정적·스크립트·파이프, 사람도 직접. **MCP 의 강점은 하나** — 등록하면 에이전트가 **자동 인식**하는 편의(대신 공식 API 범위·공식 키·MCP 호스트 필요). 그래서 **에이전트에 손 안 대고 물려두려면 MCP, 그 외 대부분은 CLI.**

| 방식 | 강점 | 대가 | 시작 |
|---|---|---|---|
| **CLI** (`tossctl …`) | **전체 기능**(공식+WTS) · **공식 키 없이 시작** · 결정적·스크립트·파이프 · 사람도 직접 | 에이전트엔 프롬프트/스킬/`AGENTS.md` 로 존재를 알려줘야 함 | 바로 아래 ↓ |
| **MCP** (`tossctl mcp`) | 등록하면 에이전트가 **자동 인식** · catalog 로 컨텍스트 최소화 | 공식 API 범위 · 공식 키 필요 · MCP 호스트 필요 | [MCP 빠른 시작 →](#mcp-빠른-시작--3단계) |
...
| 공식 키 | **불필요** — 웹 세션만으로 전부 동작(연결 시 자동 라우팅) | **필요** — 공식 API 전용이라 `openapi login` 필수 |
