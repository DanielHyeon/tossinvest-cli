...
**tossctl 은 두 가지 방식으로 연결합니다 — 호스트에 맞게 고르세요.**

| 방식 | 무엇 | 시작 |
|---|---|---|
| **CLI** (`tossctl …`) | 셸 명령으로 실행. 터미널·스크립트·cron, 그리고 **셸을 쓰는 AI 에이전트**(Claude Code·Codex·Cursor…). **전체 기능**(공식+WTS) | 바로 아래 ↓ |
| **MCP** (`tossctl mcp`) | 구조화된 MCP 툴로 노출. **MCP 네이티브 호스트**에서 에이전트가 툴로 호출 — catalog 로 컨텍스트 최소화. **공식 API 범위** | [MCP 빠른 시작 →](#mcp-빠른-시작--3단계) |

**AI 에이전트는 둘 다 곧잘 씁니다** — CLI 는 셸 명령으로, MCP 는 구조화된 툴로. 자연어로 다루는 건 어느 쪽이든 동일하고, 차이는 **연결 방식과 커버 범위**입니다. 자세한 비교: [CLI 와 MCP — 언제 무엇을](#cli-와-mcp--언제-무엇을-상호-보완).
...
- **AI 에이전트는 둘 다 곧잘 씁니다.** MCP 가 있어야 에이전트가 쓰는 게 아닙니다 — 셸을 다루는 에이전트(Claude Code·Codex·Cursor)는 CLI 를 이미 잘 실행합니다(설치만 하면 **전체 기능**). MCP 는 셸 없이 **툴로 붙이는** 경로이고, catalog 로 상시 컨텍스트를 아끼며 **공식 API 범위**를 노출합니다.
