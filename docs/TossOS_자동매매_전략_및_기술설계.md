...
작성 기준일: 2026-08-11  
근거 기준: 첨부 TossOS 분석 문서, 2026-07 StockOS·TossOS 실측 기록, 공식 시장·브로커·연구 자료

## 1. 최종 결론

TossOS는 새로운 매매 아이디어를 더 넣기 전에 **현재 강점인 원장·복구·Guardian·자동청산을 자동진입과 안전하게 연결하는 일**이 우선이다. 현재 확인된 상태는 다음과 같다.

- 실계좌 원장, 재시작 복구, 외부 보유 편입, reduce-only 자동청산은 이미 운영 경로에 있다.
- Guardian도 프로덕션 엔진에 연결되어 있다.
- 후보 자동 수집은 존재하지만 `seen_late`와 `extended` 승인 임계값이 없어 `passed=0`인 상태다.
- 후보 verdict를 매수 주문으로 소비하는 프로덕션 진입 루프와 전략 레인이 없다.
- 정정·취소는 2026-07-29 실계좌 재측정에서 브로커 수락까지 확인됐지만, 이것만으로 **신규 체결 직후 브로커 상주 보호주문이 항상 설치된다**고 볼 수는 없다.
- 성과 모델은 비용 후 손익, R, markout, MFE/MAE를 표현할 수 있지만 운영 projector가 완전한 체결·비용·FX 데이터를 만들어내지 못한다.
...
| 구분 | 내용 | 설계에서의 취급 |
| --- | --- | --- |
| 확인된 사실 | TossOS 후보 수집은 구현됐지만 후보 통과와 매수 소비자가 없다 | 진입 엔진의 선행 결함으로 처리 |
...
초기 독립 레인은 **3개**가 적절하다. 더 늘리면 적은 실거래가 여러 버전으로 분산되어 어느 전략이 좋은지 판정하기 어려워진다. 각 레인은 KR과 US 파라미터 버전을 따로 갖되 동일한 상태 계약을 사용한다.

### 6.1 S1 — Breakout Retest Confirmed

목적: 돌파 직후 추격하지 않고, 완성봉 돌파와 첫 유효 리테스트 후 재탈환을 매수한다.

상태 흐름:

`DISCOVERED → BREAKOUT_CLOSED → RETEST_WAIT → RECLAIMED → ARMED → SUBMITTED → FILLED`
...
- breakout close buffer: `max(1 tick, 0.10 ATR)`
- retest tolerance: `0.10~0.25 ATR`
- retest timeout: KR 1분봉 8개, US 1분봉 10개
- RVOL: 1.5 이상을 v1 LIVE 조건으로 사용하되 1.2/2.0/2.5의 반사실 결과도 저장
- 긴 윗꼬리 veto: upper-wick/range 0.35 초과

이 수치는 시장의 법칙이 아니다. 첫 버전을 재현 가능하게 만들기 위한 사전 고정값이다.

무효화:

- 기준선 아래 종가 마감
- 리테스트 구간에서 거래량만 증가하고 재탈환 실패
- timeout
- 진입 전 스프레드 급증 또는 가격 drift 초과

### 6.2 S2 — Momentum Pullback Reclaim

목적: 이미 모멘텀이 확인된 종목을 세션 고가에서 사지 않고, VWAP·돌파선·단기 EMA 중 사전 지정된 지지대 눌림과 재탈환에서 매수한다.

초기 조건:

1. L1 순위의 신규 진입 또는 순위 가속도 조건 충족
2. 시간대 보정 RVOL과 거래대금 적격
3. 직전 impulse 이후 고점 대비 눌림이 `0.5~1.5 ATR` 범위
4. 눌림 저점이 구조적 무효화선을 깨지 않음
5. 지지대 재탈환 완성봉 또는 실시간 호가 확인
6. day-high distance가 지나치게 가깝거나 overextended이면 차단

S1과의 차이는 명확하다. S1은 ‘저항 돌파→리테스트’라는 가격 구조가 필수이고, S2는 ‘기존 모멘텀→질 좋은 눌림→재개’를 거래한다. 두 레인의 신호가 동시에 발생하면 중복 주문을 만들지 않고, 사전 정의한 우선순위와 하나의 `entry_decision_id`만 선택한다.

### 6.3 M1 — Medium Trend Pullback Continuation

목적: 단기 급등이 아니라 일봉 추세가 유지되는 종목의 며칠짜리 눌림 후 재개를 거래한다. 12주 가치검증을 선행하지 않아도 소액 LIVE로 시작할 수 있지만, 장중 레인과 보유기간·손절·자본 슬롯을 분리한다.

초기 조건:

1. 일봉 20일 추세와 60일 추세가 같은 방향
2. 시장 또는 섹터 대비 상대강도가 양수
3. 최근 고점 추격이 아니라 20EMA, 직전 돌파대 또는 volume-profile 지지 구간으로 눌림
4. 일봉 또는 30분봉 재탈환 확인
5. 실적 발표 등 예정 이벤트가 가까우면 신규 진입 금지 또는 전용 위험 프로필 사용
6. 최대 보유기간과 time stop을 주문 전에 확정

이 레인은 단기 레인보다 stop이 넓으므로 같은 주문금액이 아니라 같은 **원화/달러 위험액**으로 크기를 정한다.

### 6.4 초기 비활성 레인 — Opening Fast Chase

개장 직후 고가 돌파를 즉시 추격하는 레인은 초기 LIVE에서 제외한다. 이유는 이론이 아니라 StockOS 실측이다. 2026-07-14 체결 5건이 모두 손절이었고 확인신호 5개 중 3개가 배선되지 않은 채 세션 고가에서 진입했다. 향후 다음이 모두 충족될 때만 새 버전으로 재개한다.
...
- 주문금액 상한: KR 190,000원, US는 주문 시점 FX로 환산한 동등 금액
...
1. **현재 HEAD 재동기화**: 첨부 분석의 commit 기준과 현재 코드를 비교하고 사실표를 갱신한다.
2. **`add-protection-orders`**: 시장별 브로커 상주 보호, 부분체결, 복구, orphan 제거.
3. **`approve-candidate-veto-thresholds`**: `seen_late`, `extended`를 측정하고 사람 승인값으로 고정.
4. **`add-strategy-engine`**: `StrategyLane` 인터페이스, 상태 머신, 멱등 decision 생성.
5. **`add-market-aware-scheduler`**: KR/US 달력, 정규장, 휴장, 장 전환.
6. **`add-multi-market-risk-envelope`**: KRW/USD, 레인 slot, global cap.
7. **S1·S2·M1 v1 구현**: 같은 인터페이스, 별도 파라미터·버전·성과 귀속.
8. **최소 실계좌 수직 슬라이스**: 후보 1개→진입 1개→보호→정정/취소→청산→잔여물 0.

### P1 — 수익성 증거 완결

첨부 분석이 제안한 순서를 유지한다.

- `a100-reconcile-profitability-evidence-contract`
- `a101-project-complete-trade-performance`
- `a102-measure-refused-entry-counterfactuals`
- `a103-precommit-strategy-promotion-criteria`
- `a104-operate-profitability-experiment`

단 실제 Story 번호는 현재 PM tracker를 다시 읽고 예약한다. 이미 사용된 번호를 추정해서 쓰지 않는다.

### P2 — 운영 확장

- REST API + SSE/WebSocket 상태 스트림
...
초기에는 S1과 S2를 KR·US 정규장에서 소액 LIVE로 운영하고, M1은 동시 포지션 1개로 제한한다. KR 기관·외국인 수급은 후보 우선순위만 높인다. 조기 고점 추격은 비활성으로 유지한다. 거래당 원금 비율보다 구조적 손절을 먼저 계산하고, 주문금액은 위험액에서 역산한다.
