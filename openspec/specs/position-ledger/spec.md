...
campaign apply hook은 기동 시 Position 투영 hook과 exit apply hook 사이에 결합되며(SHALL), 그 순서는 Position이 수량 권위를 먼저 확정한 뒤 campaign이 lineage만 기록하도록 보장한다.
...
campaign command와 event 테이블의 append-only 성질은 DB 트리거로 강제되어야 하며(SHALL), 애플리케이션 코드의 두 번째 판정으로 대체해서는 안 된다(MUST NOT).
...
#### Scenario: campaign event 수정 시도
- **WHEN** 이미 기록된 campaign command 또는 event를 UPDATE하거나 DELETE하려 한다
- **THEN** DB 트리거가 그 문장을 거부한다
