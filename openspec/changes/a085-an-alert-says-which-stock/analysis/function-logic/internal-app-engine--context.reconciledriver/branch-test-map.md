...
|---|---|---|---|
| B1 | (349) `if` — if c == nil { | 패키지 suite (기존 커버리지) | 측정: 이 change의 테스트는 닿지 않는다. 패키지 전체 suite가 덮는다 |
| B2 | (352) `if` — if !c.Automation.Verified { | 패키지 suite (기존 커버리지) | 측정: 이 change의 테스트는 닿지 않는다. 패키지 전체 suite가 덮는다 |
| B3 | (364) `if` — if opts.Prices == nil { | 없음 | **측정: 어떤 테스트도 이 줄을 실행하지 않는다** |
| B4 | (367) `if` — if opts.Alerts == nil && c.Notifier != nil { | 없음 | **측정: 어떤 테스트도 이 줄을 실행하지 않는다** |
| B5 | (370) `if` — if opts.Log == nil { | 없음 | **측정: 어떤 테스트도 이 줄을 실행하지 않는다** |
