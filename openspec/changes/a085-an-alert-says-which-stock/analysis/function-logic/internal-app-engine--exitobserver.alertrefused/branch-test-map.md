...
> 미검증 분기: B2. 이 change가 바꾼 동작이 아니거나 테스트 하네스가 구성할 수 없는 상태다.
> 관측하지 않은 것을 관측했다고 적지 않는다 — 네 번째 독립 리뷰가 잡은 것이 그 행들이다.

| Branch | Condition | Covering test | Measured |
|---|---|---|---|
| B1 | (1517) `if` — if o.refused[m.position.ID] { | `TestAQuarantineThisSelectorWroteIsStillSkipped` | 측정: 아래 테스트가 이 줄을 실행한다 |
| B2 | (1523) `if` — if errors.As(cause, &refusal) { | 없음 | **측정: 어떤 테스트도 이 줄을 실행하지 않는다** |
