...
|---|---|---|---|
| B1 | (1572) `if` — if !running { | 패키지 suite (기존 커버리지) | 측정: 이 change의 테스트는 닿지 않는다. 패키지 전체 suite가 덮는다 |
| B2 | (1576) `if` — if now.Sub(since) < o.delayBound() \|\| o.delayAlerted[m.position.ID] { | 패키지 suite (기존 커버리지) | 측정: 이 change의 테스트는 닿지 않는다. 패키지 전체 suite가 덮는다 |
