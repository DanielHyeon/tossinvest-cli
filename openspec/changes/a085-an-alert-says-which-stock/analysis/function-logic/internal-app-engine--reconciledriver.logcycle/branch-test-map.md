# Branch Test Map: `ReconcileDriver.logCycle`
...
|---|---|---|---|
| B1 | (526) `if` — if d.opts.Log == nil { | 패키지 suite (기존 커버리지) | 측정: 이 change의 테스트는 닿지 않는다. 패키지 전체 suite가 덮는다 |
| B2 | (529) `if` — if cycle.Err != nil { | 없음 | **측정: 어떤 테스트도 이 줄을 실행하지 않는다** |
