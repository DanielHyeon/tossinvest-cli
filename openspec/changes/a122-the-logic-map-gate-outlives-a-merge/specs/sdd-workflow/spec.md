...
- **AND** gate 가 띄우는 git 은 교체 참조(`refs/replace`)를 따르지 않으며, 저장소가 gate 가 해시한 바이트와 다른
  내용을 같은 이름(oid)으로 내놓아 diff 가 그 편집을 보여 주지 않으면 gate 는 그 파일의 이름을 대고 거절해야 한다(SHALL)
