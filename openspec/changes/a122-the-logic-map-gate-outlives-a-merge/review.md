...
| 적대 | `diff.noprefix` · `GIT_LITERAL_PATHSPECS` 는 요구를 지우고 `diff.mnemonicPrefix` · `color.ui=always` · `"` 든 이름은 거짓 차단 (7.5.31 정정: `noprefix` 는 `a/`·`b/` 로 시작하는 경로에서만 — 거짓 차단 · 엉뚱한 경로 · 미끼가 있으면 빔; `color.ui` 는 base 에서 조용히 `[]` 였다; 7.5.34 정정: `GIT_LITERAL_PATHSPECS` 의 "요구를 지운다" 는 ff19be01 에서 거짓이었다 — 7.5.31 의 규칙 2 가 "the object store" 탓으로 **거절**했다, 주장정확성 재리뷰 실측) | 형식·pathspec 의 문제라 스냅숏이 안 닫는다 → **7.5.26** 에 실었다(pathspec 쪽은 7.5.34 가 닫았다 — 목록과 두 diff 가 pathspec 을 안 받는다) |
...
## 정정 · VERIFY — task 7.5.31 (7.5.25 재리뷰 수리, 2026-09-24)

**적대 재리뷰가 새 조용한 길 셋을 냈다 — 셋 다 7.5.25 가 연 회귀다.** 스냅숏은 디스크 바이트를 바르게 읽는다. 그러나
두 diff 가 그 blob 을 **oid 로** 다시 찾을 때 git 은 저장소를 믿는다. 7.5.25 가 "git 의 투영을 안 믿는다" 고 적은 것은
워킹트리 투영에 대해서만 참이었다 — 저장소 투영(교체 참조 · 객체 저장소)이 같은 계열로 남아 있었다.

| # | 모양 | `cfe8d08b` | `9bc103fa` | 7.5.31 |
|---|---|---|---|---|
| F1 | `refs/replace/<편집 blob>` → base blob, 참조 하나 | 잡음 | **`[]`** (내가 재현) | 잡음 — `GIT_NO_REPLACE_OBJECTS=1` |
| F2 | 스테이지한 편집의 loose 객체를 base 바이트로 위조 | 잡음 | **`[]`** | 이름 댄 거절 — git 밖 대조 |
| F3 | 편집 oid 의 pack 항목을 base 바이트로 위조(스테이지 안 함) | 잡음 | **`[]`** | 이름 댄 거절 — git 밖 대조 |
| F4 | base blob 을 편집 blob 으로 교체 | `[]` | `[]` | 잡음 — 같은 환경 변수 |
| F9 | 이름이 UTF-8 아닌 **안 바뀐** `*.go` | 통과 | 이름 없는 해독 오류 | 통과 — `os.fsdecode` |
| F10 | 저장소 경로에 `:`·`"` | 통과 | 거절 | 통과 — C 인용 |

**F3 의 픽스처.** `git pack-objects` 는 loose 객체의 해시를 다시 확인하지 않고 담는다 — 위조 loose 객체를 만들고 pack 으로
옮긴 뒤 loose 를 지우면 결정적인 위조 pack 이 된다(/tmp 에서 `git cat-file -p <편집 oid>` 가 base 바이트를 냄을 먼저
확인했고, 시험도 그 양성 대조를 단언한다).

**대조가 git 밖에 있어야 하는 까닭.** 두 diff 는 같은 저장소를 읽으므로 둘을 서로 맞대는 7.5.23 의 교차 검사는 원리상
못 본다. 한쪽은 Python 이 바이트에서 계산한 oid 다. 믿는 것은 base 트리 하나다(그 위조는 7.5.33).

**위조 시험에서 git 이 한 일.** 규칙 3(`0`/`0` 을 거절)을 빼는 변이 `AI7` 은 직접 시험 하나만 잡았고, 규칙 1(레코드에
없음)을 빼는 `AI8` 은 위조 시험도 잡았다 — oid 가 달라도 내용이 같으면 git 이 그 경로를 numstat 에 **아예 안 냈다**.
그래서 규칙 셋을 레코드를 직접 건네 재는 시험을 따로 뒀다(`test_the_store_cross_check_reads_each_rule_directly`).

**변이가 코드를 지웠다.** `AI10`(ls-tree 에서 blob 종류만 모으는 거름을 뺌)이 살아남았다. 동등이라 적지 않고 닿는
길을 쟀다: base 의 gitlink `sub.go` 가 인덱스에 남아 있으면 스냅숏이 `mode 160000` 으로 먼저 거절하고, 지우면 numstat 이
`0\t1\tsub.go` 로 삭제를 낸다 — 거름이 판정을 바꾸는 입력이 없다. 줄을 지웠다(KISS).

**RED → GREEN.** 새 시험 여섯(8 경우): 교체 참조 둘 · 위조 저장소(loose · pack) · `:`/`"` 경로(2 경우) · 비 UTF-8 이름 ·
규칙 직접 시험. 부모 `9bc103fa` 에 붙이면 8 경우 전부 빨갛다(실패 4 · 오류 4). 그 판에서 5 경우가 더 빨갛다 — 스냅숏이
셋째 값(`placed`)을 돌려주게 되면서 튜플을 둘로 풀던 시험 둘과 대역 스냅숏을 쓰는 mock 시험 셋이다(계약 변경).
7.5.25 의 `:` 거절 시험은 통과 시험으로 바꿨다. 편집 후 **362** 전부 초록.

**정정 — 주장정확성 재리뷰 (같은 실수 일곱 번째).**

| # | 쓴 것 | 실제 |
|---|---|---|
| C1 | BTM AH2 "21 시험" | 22 — 같은 커밋에서 뒤에 더한 `…untouched_worktree…` 도 잡는다. 7.5.31 창에서는 27 |
| C2 | BTM AH15 "91 시험" | 92, 같은 까닭. 7.5.31 창에서는 97 |
| C3 | 7.5.26 "`diff.noprefix` 가 요구를 조용히 지운다" | **재지 않았고 틀렸다.** `a/`·`b/` 로 시작하는 경로에서만 — 거짓 차단 · 엉뚱한 경로 · 미끼가 있으면 빔 |
| C4 | 7.5.26 "`color.ui=always` 는 이전부터 거짓 차단" | base 에서는 조용히 `[]` |
| C5 | 7.5.26 `mnemonicPrefix` 는 `c/`·`w/` | `--cached` 뒤로 `c/`·`i/` |
| C6 | 가드 FLM "직접 부르는 자리 하나" | 셋 |
| C7 | 몸통 docstring "스냅숏이 열려 있는 동안 돈다" · README clean 필터 "늘" | 커밋 대상은 스냅숏 없이 돈다 · 항등 필터면 같다 |

일곱째 재발의 모양은 여섯째와 **같다**: 뒤에 더한 시험이 앞에 옮겨 적은 수를 낡게 했다. 이번 로트는 변이 판 **뒤에**
시험을 더하지 않았고, 7.5.25 의 N 시험 행을 이번 판에서 다시 옮겼다. C3 은 다른 모양이다 — 리뷰가 적은 문장을 재지 않고
사실로 옮겼다.

**변이.** 171(AI1~AI12 더함 · AH25 는 겨눌 거절이 없어 뺌 · AH15/AH19 앵커 갱신) — `0:43` · `43:86` · `86:129` 각 **43/43**,
`129:171` **41/42**(AI10). AI10 을 지운 뒤 하네스는 **170**. 모든 창 대조군 양끝 GREEN(`Ran 362`).

**게이트.** `make lint` rc 0 · `make sdd-test` rc 2 — **logic-map 437 통과**(357 → 362 에 스위트 밖 시험 포함 432 → 437); 멈춘 곳은 `tools/sdd` 의 `test_repository_contract_is_synchronized` 하나("Claude/Codex minimal safety bootstrap drift") — 다른 세션이 커밋하지 않은 `.claude/CLAUDE.md` · `.codex/agents.md` 편집 탓이다(깨끗한 클론 `9bc103fa` 에서는 통과, 이 로트는 두 파일을 안 건드리고 커밋에도 안 넣었다). 멈춘 뒤의 묶음은 따로 돌렸다: `sdd-history` 22 · `pm` 16 · `deploy` 18 · `go test ./tools/logic-map` 전부 통과 · `make test-seams` rc 0(**108 패키지**) · `openspec validate --all --strict` **58/58** ·
a122 게이트 **rc 0**(a112 `required 64` 그대로) · 좌표 게이트 **rc 0**.
...
### 안 한 것 — 사유
- **7.5.32**(파서를 base 에서 빌드): 판정 비용과 기존 번들의 재현성이 바뀌는 설계 변경 — 사람 결정.
- **7.5.33**(base blob 을 Python 으로 해시해 트리 oid 와 대조): `base_file` 내부 변경이라 FLM 이 먼저고, 트리 · 커밋 위조는
  그것으로도 안 닫힌다.
- **16 MiB 를 넘는 안 바뀐 `*.go` 의 거절**: 깔때기의 상한이다. 저장소 최대 97 KB, 노출 0 — README 에 적었다.

## 정정 · MEASURE · VERIFY — task 7.5.34 (판정 blob 을 실제 저장소에서 찾지 않는다, 2026-09-25)

**재리뷰 둘(ff19be01).** 적대: 7.5.31 의 git 밖 대조는 **있음**만 본다 — (#1) 편집 oid 에 base 도 디스크도 아닌 바이트 →
`[]` 또는 엉뚱한 함수, (#2) `git mv` + base 바이트 위조 → 100% rename `0`/`0` → `[]`(삭제도 숨는다), (#3) 빈 새 `.go`
헛거절, (#4) 주 저장소 경로의 비 UTF-8 → 해독 오류, (#5) 그 경로의 줄바꿈 → 대체 저장소 경로가 잘림. 주장정확성: 틀린
문장 넷(아래), 그리고 base 쪽 구멍(7.5.33)이 미끼 변형으로 ff19be01 에서도 `[]`. 사람이 선택지 1(class 수리)을 골랐다.

**수리.** 판정이 읽는 바이트의 출처를 둘로 줄였다 — 디스크(게이트가 해시)와 **검증한 객체**(`_verified_objects`: `cat-file
--batch` 의 대답을 `"<종류> <크기>\0" + 바이트` 로 해시해 oid 와 같아야 받는다). base(와 커밋 대상)는 커밋 → 트리 → blob 을
검증해 걷고(`_verified_go_entries`), 두 쪽 `*.go` 만 담은 트리를 **대체 저장소 없는** 임시 저장소에 세워(`_isolated_tree` ·
`_isolated_comparison`) 두 diff 가 그 트리 둘을 견준다. 지운 것: `_snapshot_disagreement` · `_c_quoted` · `_compared` ·
`base_file` · `_worktree_snapshot`(읽기 절반은 `_worktree_entries`). 목록과 두 diff 는 pathspec 을 안 받는다.

| 모양 | ff19be01 | 7.5.34 |
|---|---|---|
| 편집 oid 위조(base 바이트, loose · pack) | 이름 댄 거절(규칙 1) | 요구가 선다 — 위조 객체를 안 읽는다 |
| 편집 oid 위조(미끼 줄, #1) | **`[]`** | 요구가 선다 |
| rename + 위조 · 삭제 + 위조(#2) | **`[]`** | 요구가 선다 |
| base blob 위조 · 미끼 · **하위 트리** 위조(7.5.33) | 거절 · **`[]`** · **`[]`** | `STORE_LIES` 거절 |
| 뿌리 트리 · 커밋 위조 | git 자신의 `hash mismatch` | 같음(git 이 먼저 말한다) |
| 커밋 대상에서 한쪽 blob 위조 | **`[]`** | `STORE_LIES` 거절 |
| 물려받은 대체 저장소 + 위조 pack | 거절 | 요구가 선다 |
| 빈 새 `.go`(#3) | 헛거절 | 통과 |
| `GIT_LITERAL/GLOB/NOGLOB_PATHSPECS` | 헛거절("object store") | 요구가 선다 |
| 연결 워크트리, 주 경로에 `\n` · `\xff`(#4 · #5) | 오류 | 요구가 선다 |

**작업 중 잰 것.** (a) `ls-tree -r -d` 는 gitlink 도 낸다 — 첫 판이 전부 트리로 물어 하위 모듈이 있는 저장소를 헛거절
했다(고침 · 시험). (b) `write-tree` 도 인덱스를 다시 써 split index 면 `sharedindex.*` 를 실제 `.git` 에 썼다(기존 시험이
잡았다 · 고정을 둘 다에). (c) git 은 하위 트리를 해시 없이 읽는다 — 하위 트리 위조는 이 검증만 잡는다. (d) 조언 시험 둘이
"base sha 로 시작하는 `cat-file`" 을 조언의 읽기로 셌는데 판정의 검증 읽기도 base 커밋부터 묻는다 — 거름을 조언의 `-Z` 로
좁혔다(판정 쪽 `cat-file` 결함은 `test_every_git_fault_of_the_comparison_is_named` 가 잰다).

**변이가 코드를 바꿨다.** AJ44(바이트 출처를 디스크 · 검증 객체 중 고르는 갈래를 뒤집음)가 살아남았다 — oid 가 같으면
바이트가 같으므로 고를 것이 없었다. 한 표(oid → 바이트)로 바꿨다. AJ47(두 diff 의 `GIT_INDEX_FILE`)이 살아남았다 — 트리
둘을 견주는 diff 는 인덱스를 안 쓰고 "쓰지 않는다" 시험도 초록이었다. 줄을 지웠다. 그 함수를 겨누는 변이는 최종 코드에서
다시 돌렸다(창 `174:187` 13/13 · `135:136` 1/1).

**RED → GREEN · 변이 · 게이트 · A/B.** 수치는 tasks 7.5.34 에 한 번 적었다(시험 362 → 372 · ff19be01 코드에서 53 경우
빨강 · 변이 189 전부 CAUGHT · A/B 126/126 SAME, git 20,825 → 2,431).

**정정 — 주장정확성 재리뷰(ff19be01), 같은 실수 여덟 번째.**

| # | 쓴 것 | 실제 |
|---|---|---|
| D1 | 규칙 직접 시험 docstring: 행동 시험만으로는 **규칙 1 · 2** 를 빼는 변이가 산다 | 규칙 1(AI8)은 행동 시험이 잡았다 — 직접 시험만 잡는 것은 **3 · 2**. 같은 커밋의 뒤 발견(git 이 그 경로를 numstat 에서 뺀다)이 낡게 했다. 시험과 함께 지웠다 |
| D2 | 7.5.26 `GIT_GLOB_PATHSPECS` "요구가 조용히 빈다" | base · 9bc103fa 에서만 참. ff19be01 에서는 규칙 2 가 **거절**("the object store" — 틀린 원인) |
| D3 | 7.5.26 · review 7.5.25 `GIT_LITERAL_PATHSPECS` "요구를 지운다" | D2 와 같다 |
| D4 | 7.5.33 "pack 을 심으면 요구가 빈다(리뷰어 재현)" | 9bc103fa 의 재현. ff19be01 에서는 이름 댄 거절 — 구멍은 미끼 변형으로 실재 |
| (작음) | 7.5.31 gitlink "워킹트리에 남아 있으면" 거절 | 인덱스 항목이 거절을 부른다 |

여덟째의 모양은 둘 다 알려진 것이다 — 같은 커밋의 뒤 발견이 앞 문장을 낡게 했고(D1), 리뷰어가 **앞 커밋**에서 잰 문장을 새
코드에서 다시 재지 않고 옮겼다(D2~D4). 이번 로트는 옮긴 수치를 전부 이 로트의 로그에서 다시 옮겼고, 기억으로 적었던
`GIT_NO_LAZY_FETCH`(2.44) 는 재지 못해 지우고 잰 것(`--no-lazy-fetch` rc 129)만 남겼다.

### 안 한 것 — 사유
- **7.5.33 의 남은 절반**(부분 클론 fetch): 판정은 맞고 쓰기가 실제 저장소로 간다. git 2.43 이 `--no-lazy-fetch` 를 몰라
  막는 방법을 따로 재야 한다 — P2 로 좁혔다.
- **7.5.32**(파서를 base 에서 빌드): 사람 결정.
- **7.5.26**(diff 출력 형식 설정): 형식의 문제라 이 수리가 안 닫는다. pathspec 쪽은 닫혔다.

## 정정 · MEASURE · VERIFY — task 7.5.35 (검증한 트리의 모양 · 판정 diff 의 형식, 2026-09-25)

**재리뷰 둘(0ece1074).** 적대: 위조 저장소 class 는 닫혔다(위조는 이름 댄 거절이거나 판정에 안 닿는다). 그러나 7.5.34 의
검증한 걷기가 git 과 **다르게** 읽는 트리가 있다 — P0-1 `040000`(0 으로 채운 트리 모드), P1-2 같은 이름 둘 · `/` 든 이름,
둘 다 7.5.34 가 연 회귀. 그리고 P0-3 `diff.noprefix` · P2 색 · `GIT_DIFF_OPTS`(noprefix 와 `-u3` 은 이 change 이전부터; 색은 이 change 의 base 에서 **조용한
`[]`** 였고 7.5.23 의 교차 검사가 헛거절로 바꿨다). 주장정확성: 틀린 문장 셋
(아래). 적대 리뷰의 픽스처가 실제 저장소에서 `git init/config/commit` 을 돌린 사고가 있었다 — 리뷰어가 HEAD · 워크트리를
되돌렸고 남은 로컬 `user.*` 는 사람이 지우라고 해서 지웠다. 사람이 선택지 1 을 골랐다.

**수리.** (1) `_verified_go_entries` 가 트리 항목마다 git 이 **쓰는** 모양인지 본다 — 기본 fsck 가 받는 모드 여섯 · git 정렬(하위 트리 열쇠는
이름 + `/`) · 유일한 이름 · 비지 않고 `/` 가 들지 않고 `.` · `..` 가 아닌 이름. 어기면 `NOT_CANONICAL`. (2) 판정 diff 에 `--src-prefix=a/
--dst-prefix=b/ --no-color --inter-hunk-context=0`. (3) `_isolated_comparison` 이 `GIT_DIFF_OPTS` 도 물려주지 않는다.

| 모양 | ff19be01 | 0ece1074 | 7.5.35 |
|---|---|---|---|
| base 트리의 `040000 sub` (워킹트리 · 커밋 대상 × sha1 · sha256) | 요구가 선다 | **`[]`** | `NOT_CANONICAL`(mode 040000) |
| 같은 이름 `x.go` 둘(base, 편집) | 요구가 선다 | **`[]`** | `NOT_CANONICAL`(duplicate entry) |
| 하위 트리에 편집 blob + 평평한 `sub/x.go` 에 base blob | 요구가 선다 | **`[]`** | `NOT_CANONICAL`(entry name) |
| `diff.noprefix` + `a/x.go` · `b/x.go` | 헛거절 · 엉뚱한 경로 | 같음 | 요구가 선다 |
| `color.ui` · `color.diff=always` | 헛거절 | 같음 | 요구가 선다 |
| `diff.interHunkContext=9`(A · C 편집) | B 까지 요구 | 같음 | A · C 만 |
| `GIT_DIFF_OPTS=-u3`(A · C 편집) | B 까지 요구 | 같음 | A · C 만 |
| 이름에 `"` · `\` | 헛거절 | 같음 | 같음(7.5.26 에 남김) |

표의 모든 칸은 이 로트에서 세 판본의 코드로 쟀다(리뷰어의 값을 옮기지 않았다). ff19be01 은 트리를 git 이 걸었으므로 앞 셋이 요구가 선다.

**거절하는 정상 입력을 먼저 셌다.** `rev-list --objects --all` 로 닿는 트리 전부에 거절 0(12 시 무렵 13,749 · 항목 218,464, 20:47 에 13,980 · 222,112 —
규칙은 측정 스크립트가 옮겨 적었다). 고정하지 않은 설정 둘
(`diff.algorithm` · `diff.indentHeuristic`)은 모호한 편집 셋에서 판정이 같았다 — 셌고 안 넣었다.

**변이가 시험을 바꿨다.** AK6(빈 이름 검사를 지움)이 첫 판에서 살아남았다: 빈 이름의 파일은 순서 검사가 같은 문장으로 대신
거절했고 시험은 문장의 앞부분만 봤다. 빈 이름의 **트리**를 더하고 모양마다 사유를 단언하게 했다 — 이 시험 수정은 병행 세션
(tossos-d6)의 팀메이트가 먼저 적었고(이 로트를 고아로 오판, 그 세션이 멈추고 소유를 확인했다) 읽고 돌려 보고 받았다. 재실행
CAUGHT. `key <= last` → `<` 는 같은 이름을 B21 이 먼저 잡아 같은 행동이라 변이에 안 넣었다.

**작업 중 잰 것.** (a) `diff.interHunkContext` 는 리뷰가 안 셌다 — 같은 class(훅 모양)라 같이 닫았다. (b) 빈 이름은 git 의
`ls-tree` 가 먼저 죽는다(`empty filename in tree entry`) — 시험은 목록을 비워 걷기의 거절을 따로 잰다. (c) 병행 세션이 같은
브랜치에 커밋해 HEAD 가 움직이며 첫 A/B 가 "HEAD moved" 로 DIFFERENT 를 냈다 — 멈추고 고정한 연결 워크트리에서 다시 돌렸다.

**이 로트의 적대 재리뷰(첫 판).** P0 · P1 없음. 고친 셋 — (P2-1, 내 회귀) 첫 판이 `100664` 를 거절했다: 초기 git 이 쓴
모드이고 기본 `git fsck` 가 받는다(모드 열 개로 실측: 기본 fsck 가 받는 것은 `40000` · `100644` · `100664` · `100755` · `120000`
· `160000` 여섯, `040000` · `100600` · `100775` · `40755` · `140000` 은 적는다) → 모드 표를 그 여섯으로. (P2-2) 붙어 있지 않은
같은 이름(`x.go` · `x.go-` · 트리 `x.go`)을 시험이 안 못 박았다 — 옆만 보는 중복 검사가 살고, 그 모양은 편집 전에 트리 `x.go/`
아래 삭제를 **조용히** 잃었다 → 시험에 더했다. (P3-1) gitlink 정렬 열쇠 → 정상 트리 시험에 이웃. (P3-2) "git 은 앞의 것을
읽는다" 정정. 셋을 변이 AK21–AK23 으로 못 박았다. 남긴 것(P3): 비표준 트리는 **Go 가 없는 디렉터리**에 있어도 거절한다 —
base 는 고정된 이력이라 막힘이 오래가지만 노출 0 이고 덜 막는 방향이 아니다.

| 모양(재리뷰 뒤) | 0ece1074 | 첫 판 | 최종 |
|---|---|---|---|
| base 하위 트리의 `100664 x.go` | 요구가 선다 | **헛거절** | 요구가 선다 |
| 붙어 있지 않은 같은 이름 `x.go` · `x.go-` · 트리 `x.go` | **트리 아래 삭제를 잃음** | 거절(시험 없음) | 거절 · 시험 |
| gitlink `b` 옆 `b-.go` | 통과 | 통과(시험 없음) | 통과 · 시험 |

0ece1074 열의 앞 둘은 이 세션이 다시 쟀다 — 붙어 있지 않은 같은 이름에서 git 의 diff 는 `x.go` 수정과 `x.go/y.go` 삭제를 내고, 편집 전 게이트는 `[('x.go', 'A')]` 만 냈다(삭제된 파일의 `A` 가 빠졌다).

**RED → GREEN · 변이 · 게이트 · A/B.** 수치는 tasks 7.5.35 에 한 번 적었다.

**정정 — 주장정확성 재리뷰(0ece1074), 같은 실수 아홉 번째.**

| # | 쓴 것 | 실제 |
|---|---|---|
| E1 | tasks 7.5.34 A/B: "116 change 가 100~230 씩 줄었다, 10 은 그대로" | 115 가 8~228 씩(100 미만 17) · 10 그대로 · **1 늘었다**(a122 14 → 18) |
| E2 | README "이 저장소 트리 4,093" | ff19be01(7.5.34 의 기준)의 하위 트리 수다 — 0ece1074 는 4,099, 걷는 트리는 a122 base 1 + 3,999 · a112 base 1 + 3,894 |
| E3 | README "`*.go` 마다 … blob 을 임시 저장소에 쓴다" | base 와 **다른** 경로의 blob 만 쓴다 |
| (작음) | tasks 7.5.34 "base 바이트 위조 둘의 헛거절" | 위조 저장소의 이름 댄 거절(규칙 1) — review 표와 맞췄다 |

아홉째의 모양: 분포의 위쪽만 보고 요약했다(E1), 앞 커밋의 수를 옮겼다(E2), 코드의 조건(다른 경로만)을 문장이 빠뜨렸다(E3).
이번 로트는 리뷰어의 수(P0-1 의 세 판본 × 두 모드 × 두 해시)를 옮기지 않고 다시 쟀다.

### 안 한 것 — 사유
- **7.5.26 의 남은 것**(`"` · `\` 든 이름의 헛거절): 저장소 노출 0, 인용된 머리 줄을 푸는 파서가 필요하다.
- **7.5.32**(파서를 base 에서 빌드) · **7.5.33 남은 절반**(부분 클론 fetch): 앞 로트와 같다.
- 병행 세션이 제안한 5.6/5.7 측정(아카이브 직전 change 다섯이 5단계에서 막힌다): 이 세션이 재지 않았다 — 사람에게 올린다.

**정정 — 이 로트의 주장정확성 재리뷰(최종 코드), 같은 실수 열 번째.**

| # | 쓴 것 | 실제 |
|---|---|---|
| F1 | README · E2: "4,093 은 어느 커밋의 수도 아니다" | ff19be01(7.5.34 의 기준)의 하위 트리 수(이 세션이 다시 쟀다) |
| F2 | FLM B19 · B20: "`/` 든 이름이면 git 은 하위 트리의 것만 본다" | git 은 둘 다 낸다(`ls-tree -r` · diff) — 편집 전 걷기가 하나로 모았다 |
| F3 | BTM: "최종 코드에서" AJ42 · AJ46 35 · AJ49 150 · AJ18 19 | 첫 판(376) 값이었다. 최종 판 36 · 151 · 20 |
| F4 | tasks · README · FLM: 색 설정은 "이 change 이전부터 헛거절" | base 7f8b1ae7 에서는 **조용한 `[]`** — 7.5.31 정정이 이미 적은 것을 되풀이했다(이 세션이 base 코드로 다시 쟀다) |
| F5 | "시험 수가 더 많이 실패한 15 변이" | 24 — 정확히 +1 인 9 는 새 시험 하나로 설명되고, 1 넘게 늘어난 15 를 다시 돌렸다 |
| F6 | README "트리 전부를 검증해 읽는다(1 + 3,999 · 1 + 3,894)" | 걷기 방문 수다. 해시를 검증하는 고유 트리 객체는 1 + 3,821 · 1 + 3,716 |
| (작음) | "브랜치 끝(8d9731c1)" · A/B 121 대조의 방법 · 전수의 시각 · `/` "인" 이름 · AK6 재실행의 시험 판 · lint 의 시각 · FLM B22 "같은 종류의 중복" · "git 의 diff 는 두 순서 모두 편집을 본다" | 제자리에 고쳤다 |
