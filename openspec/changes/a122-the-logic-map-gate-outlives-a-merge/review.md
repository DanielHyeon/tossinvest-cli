...
| 적대 | `diff.noprefix` · `GIT_LITERAL_PATHSPECS` 는 요구를 지우고 `diff.mnemonicPrefix` · `color.ui=always` · `"` 든 이름은 거짓 차단 (7.5.31 정정: `noprefix` 는 `a/`·`b/` 로 시작하는 경로에서만 — 거짓 차단 · 엉뚱한 경로 · 미끼가 있으면 빔; `color.ui` 는 base 에서 조용히 `[]` 였다) | 형식·pathspec 의 문제라 스냅숏이 안 닫는다 → **7.5.26** 에 실었다 |
