...
시험품질 리뷰어가 양방향으로 반증했다(`_reads_moved` 만 죽이면 원장 겨냥 시험 **11개**가 정확히 빨개지고,
...
`validate_target` 안에서만 참이다. `_verdict:2159` 가 `ast.json` 의 `source_sha256` 을 base 소스에서 뽑은
...
꺼낸 **파일 바이트**와 대조: **MATCH 248 · STALE 18**(a063 2 · a112 16). 그러니까 `validate_target` 에
파일 수준 해시 대조를 넣으면 **오늘 정상인 번들 18개를 새로 거절**한다 — 그중 16 은 지금 이 브랜치에 살아 있는
a112 의 것이다. 이 change 밖의 판정을 바꾸는 규칙 변경이라 7.5.4 · 7.5.6 과 같은 **사람 결정**으로 넘긴다 →
...
`hunk()` 한 곳으로 모았다. **새 거절은 0 개다** — 본문에서 이름을 안 읽을 뿐이다. 판정이 느슨해지는 방향은 없다:
...
**게이트.** `make lint` rc 0 · `make sdd-test` rc 0(logic-map **380**) · `make test-seams` rc 0(46 패키지) ·
