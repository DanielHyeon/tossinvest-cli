...
- **푸시 상태(2026-09-20)**: 원격 브랜치 tip 은 `fc35eb2d` 의 **부모** `e9f905bd` 까지 올라가 있고, 그 위에 이 change 의 미푸시 커밋이 쌓인다(`fc35eb2d` 7.5.2 · 그 다음 7.5.2.1). 푸시는 사람이 요청할 때만 한다 — `git rev-list --left-right --count origin/<브랜치>...HEAD` 로 확인할 것.
...
- 마지막 검증(7.5.2.1, 커밋 전 실행 — 수치는 review.md `## VERIFY — task 7.5.2.1`): `check_analysis --change a122` **rc=0** ·
  `openspec validate --all --strict` **58/58** · `make sdd-test` · `make lint` · `make test-seams` · `test_check_analysis` **254**.
