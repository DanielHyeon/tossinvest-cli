...
## 편집 — task 7.6 (규칙 한 집) · 분기 42 → 42 · 반환 2 → 2 · raise 24 → 24

편집 전 `ast.before-7.6.json`(HEAD `2b5b05c1`) · 편집 후 `ast.after-7.6.json`(워킹트리). 아래 표는 두 열거의
`source` 를 스크립트가 줄 단위로 대조한 것이다.

지역 이름 `canonical` → `recorded_dir`(I3). 모듈 함수 `canonical()` 을 가리던 덫이다. 분기 42 · 반환 2 · raise 24 전부 같고 소스 한 줄 대조에서도 차이가 0 인 것은 이름이 분기·raise 줄에 안 나오기 때문이다.
