...
## task 7.5.12 — 인용 경로의 풀이도 원장에 (2026-09-27)

> 편집 전 `ast.before-7512.json`(HEAD `5406cac1`) · 편집 후 `ast.after-7512.json`(최종 `check_analysis.py` `463982f42151`) — 분기 9 → 9(정렬 결과 전부 같음), 호출 7 → 6.

B2 의 `os.path.realpath` 둘을 `_resolved` 로(같은 부류 — 7.5.12). 행동 시험은 없다(판정 중 인용 경로의 심링크를 갈아끼우는 모양은 안 만들었다) — 구조 시험이 이 호출
형태가 깔때기 밖에 없음을 못 박는다(변이 MW9).
