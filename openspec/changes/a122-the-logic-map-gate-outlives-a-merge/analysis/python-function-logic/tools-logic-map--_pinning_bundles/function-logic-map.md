...
편집 전 L621-623 (`ast.before-7.5.2.1.json` = revision `fc35eb2d`) · 편집 후 없음.

지웠다(task 7.5.2.1) — 호출자 셋이 전부 한 번 읽은 `Evidence` 를 `_select_pinning` 에 넘긴다. 디스크를 스스로 읽는 마지막 선별 철자였다.
