...
`tools/logic-map/check_analysis.py:612-627` · 분기 3 · 반환 2 · raise 1 (편집 전 L576-591 · 분기 3 · 반환 2 · raise 1, `ast.before-7.5.2.1.json` = revision `fc35eb2d`).
...
| id | 줄 | 종류 | 소스 |
|---|---|---|---|
| B1 | 621 | If | `if record is None:` |
| B2 | 624 | Try | `try:` |
| B3 | 626 | ExceptHandler | `except UnicodeDecodeError as exc:` |

| raise 줄 | 소스 |
|---|---|
| 627 | `raise ValueError(f'landing point is not UTF-8: {relative}') from exc` |
