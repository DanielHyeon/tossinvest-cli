# Branch Test Map: `_remember` (Python, a122 task 7.5.2.3)
...
|---|---|---|
| 한 판에서 갈린 읽기는 그 자리가 움직임 | AA17 | `test_a_path_read_twice_with_different_bytes_is_caught_where_it_diverges` |
| 재확인 중에는 적지 않는다 | — | 구조: `_reads_moved` 가 `recording` 을 내렸다 올린다 · 행동으로는 재확인이 스스로를 못 깨운다(`test_prose_rewritten_after_it_was_judged_asks_for_a_rerun` 이 한 줄만 낸다) |
