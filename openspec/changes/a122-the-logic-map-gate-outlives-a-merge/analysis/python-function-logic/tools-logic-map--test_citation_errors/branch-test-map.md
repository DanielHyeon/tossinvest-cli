# Branch Test Map: `test_citation_errors` (Python, a122 task 7.5.2.3)
...
|---|---|---|
| 줄 수 읽기도 깔때기로 | AA3 | 기존 인용 시험 전부(`TestNamedTestsAreOpened`) |
| 고른 뒤 사라지면 그 줄엔 주장하지 않는다 | — | **경합 전용 갈래** — 주입 없이는 못 닿는다. 그 변화 자체는 원장이 잡는다(`test_a_test_file_that_appears_after_the_index_was_built_asks_for_a_rerun` 과 같은 기전) |
...
| 갈래 | 변이 | 잡는 시험 |
|---|---|---|
| 거절 문장 "in any tracked file" | AM8 | `test_a_test_only_in_an_untracked_file_is_not_evidence`(문장 전체 일치) |
| 해소에 `index.files` 를 넘긴다 | AM2 · AM3 · AM4 가 대신 잰다 | 위 표 |

분기 8 은 편집 전후 같다 — 이전 표가 그대로 선다.
