...
## task 7.5.13 — 주석만 (2026-09-27)

> 편집 전 `ast.before-7513.json`(HEAD `26e5bb3f`) · 편집 후 `ast.after-7513.json` — 분기 13 → 13, 반환 · raise · 호출 수 같음(정렬 결과 전부 같음).

`\n` · `\r` · `\t` 거절 옆에 주석을 더했다: 판정의 이름은 이제 머리 줄이 아니라 레코드에서 오므로 `"` · `\\` · 나머지 제어 문자는 받고, 이 세 글자는 판정 줄에
줄바꿈 · 탭이 들어가서 거절을 그대로 둔다(열린 7.5.41). 로직 변경 0.
