...
- [x] 7.5.24 **P0 — 내가 넣은 교차 검사가 정상 입력을 거절했다 (7.5.23 재리뷰 2026-09-23, 적대 · 주장정확성).**
      ...
      **닫음(2026-09-23). 생산 Go 변경 0.** 수리: 짝을 **이름이 아니라 순서로** 짓는다. 두 호출은 같은
      ...
      이쪽이 먼저 잡는다 — 실측).
      **경계 둘을 새로 못 박았다**: 건너뛰기 조건의 `and` 를 `or` 로 바꾸는 변이가 326 시험을 **전부
...
- [ ] 7.5.26 **P2 — 파서가 `a/`·`b/` 접두사를 글자로 믿는다 (7.5.23 재리뷰 2026-09-23, 적대).**
      ...
      `c/`·`w/` 를 내서 게이트가 **아예 못 돈다**(fail-closed, 이 change 이전부터).
      저장소 노출 0(`git ls-files | grep -cE '^(a|b)/'` = 0). 근본은 접두사를 글자로 믿지 말고
      `--src-prefix=`·`--dst-prefix=` 로 **우리가 정하는 것**이다.
- [ ] 7.5.25 **P0 — 워킹트리를 다시 쓰는 것은 두 투영을 **함께** 거짓말하게 한다 (7.5.23 재리뷰 2026-09-23,
