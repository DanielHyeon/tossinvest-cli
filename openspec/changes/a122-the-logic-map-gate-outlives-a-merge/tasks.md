...
- [ ] 6.1.2 **결정 — 사람이 고른다.** 6.1.1 이 네 축의 비용을 실수로 적었다. 규칙을
      지어내지 않고 대기한다. 고르고 나면 spec 본문(`착지 지점에서 그 change 의 모든
      `revision: current` 증거 묶음의 source hash 가 일치해야 한다(SHALL)`)이 함께
      바뀌어야 한다 — 오늘 그 문장이 위조를 허용하는 판정을 `SHALL` 로 적고 있다.
