...
- [x] 3.2.2 `check` 의 `change_dir`(`check_analysis.py:590`)을 아카이브 인식으로 바꾼다.
      ...
      말고 공유한다.
- [x] 3.2.3 인용된 테스트 이름의 탐색 범위를 정한다. 지금은 착지 지점을 줘도
      `test_index(root)` 가 **워킹트리**를 훑는다. 증거가 착지를 기술하면 그 인용도
      착지에서 찾는 것이 일관되지만 spec SHALL 은 source hash 만 말한다 — 넓힐지
      결정하고 사유를 적는다.
      **종결(2026-09-27) [감사·재현성] — 착지 시점의 시험 색인 재현은 제외. 범위는 7.5.9 가 정했다(추적 파일 목록 · 워킹트리 바이트) — 넓히지 않는다.**
...
- [x] 5.1 a074 · a077 · a079 는 각자의 배포 후 실측이 남아 있어 이 change 로 닫히지
      ...
      **종결(2026-09-27) [다른 change 몫] — a074 · a077 · a079 의 배포 후 실측은 각자의 change 가 닫는다. 이 change 에서는 기록으로 닫는다.**
- [x] 5.2 2026-09-08 에 a075 · a076 이 5단계를 사유를 적고 면제한 기록(각 review 의
      §완료 게이트 5단계 면제)을 소급해 고치지 않는다. 그 면제는 당시 사실이다.
      **종결(2026-09-27) [기록] — 소급하지 않는다는 결정 그대로. 닫는다.**
- [x] 5.3 **고정의 세기는 이 change 가 답하지 않는다 (3.2.3.1 에서 열림).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — 고정의 세기(번들 수 · 구간 폭)는 필요해지면 실데이터로 잰다. 오늘 판정을 틀리게 하지 않는다(좁힘을 못 할 뿐).**
- [x] 5.4 **`revision: base` 번들만 있는 change 는 착지를 고정할 수 없다.**
      ...
      **종결(2026-09-27) [극단 edge] unsupported — `revision: base` 번들만 있는 change 는 착지를 못 고정한다(저장소 0 건). 3.2.3.1 이 선언을 거절한다(막는 쪽).**
...
- [x] 5.6 **번들이 0 인 change 는 창을 좁힐 수 없다 (4.1 에서 실물로 나왔다).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — 번들 0 인 change 는 넓은 창으로 판정된다(막는 쪽). 오늘 a076 1 건.**
- [x] 5.7 **base 가 작업 뒤에 놓인 change 는 5단계가 아무것도 요구하지 못한다
...
## 6. 독립 적대 리뷰가 연 것 (task 4.4)
      **종결(2026-09-27) [다른 change 몫] — base 배치는 각 change 의 결과이고 a122 는 그것을 보이게 할 뿐이다. 닫는다.**
...
막기)를 골랐다.
      **종결(2026-09-27) [결정 · 재분류] — (a) 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 1(hard gate 추가 안 함 · warning 도 안 만든다) · (g) [개발 편의] backlog · (h) 닫음(앞 로트) · (i) 심링크 change 디렉터리 · (j) 해소기 표 밖 불일치 · (k) root ≠ toplevel [극단 edge] unsupported · 커밋된 base 재기록 잠금은 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 8(git history 가 정본, 잠금 subsystem 안 만듦). 부모를 닫는다.**
...
- [x] 7.5.32 **P0(이 change 이전부터) — 함수 지도를 뽑는 파서를 판정받는 워킹트리에서 빌드한다 (7.5.25 적대 재리뷰 F6).**
      ...
      **종결(2026-09-27) [결정] 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 6 — current parser 를 쓰고 historical parser 를 다시 빌드하지 않는다. 닫는다.**
- [x] 7.5.33 **P2(이 change 이전부터) — 부분 클론에서 게이트가 base blob 을 가져온다 (7.5.25 적대 재리뷰 F12). 위조 절반은 7.5.34 가 닫았다.**
      ...
      **종결(2026-09-27) [극단 edge] unsupported — 부분 클론(`--filter=blob:none`)에서의 promisor fetch. 판정은 맞다.**
...
- [x] 7.5.26 **P2 — 파서가 `a/`·`b/` 접두사를 글자로 믿는다 (7.5.23 재리뷰 2026-09-23, 적대).**
      ...
      **닫음(2026-09-27, 잔여).** 7.5.13 과 같은 수리로 닫혔다 — `"` · `\` 가 든 `*.go` 는 이제 제 이름으로 판정된다(헛거절 없음). 닫힌 범위:
      ...
      믿지 않고 렌더링과 통째로 대조한다.
- [x] 7.5.30 **P2 — 시험 스위트가 공유 go 빌드 캐시를 판마다 약 375 MB 씩 불린다 (7.5.25 변이 중 실측, 2026-09-24).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — 하네스는 이미 `-trimpath` + 전용 캐시(7.5.30 측정 뒤). 스위트 자체의 캐시 성장은 개발 기계 문제.**
...
- [x] 7.5.4 **P0(앞 로트 전부터) — `ast.json` 의 구조가 소스에서 다시 유도되지 않는다 (7.5.2.1 재리뷰, 보안 전문가
      ...
      **종결(2026-09-27) [결정] 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 3 — 기존 `ast.json` 을 쓰고 구조를 자동 재유도하지 않는다. 닫는다.**
- [x] 7.5.5 **INVESTIGATE(앞 로트 전부터) — `base-commit.txt` 를 워킹트리에서 읽는다 (같은 재리뷰, 적대).** base 는
      proposal freeze 에 고정되고 spec 이 불변을 요구하지만, 게이트는 커밋이 아니라 디스크의 글자를 읽는다 — 커밋 안 한
      편집이 창의 시작을 옮길 수 있는지 재 볼 것.
      **종결(2026-09-27) [결정 · 극단 edge] — 커밋 안 한 편집 · 이동은 6.4(b) · 보수가 막는다. 커밋된 재기록은 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 8(잠그지 않는다). 남은 심링크 디렉터리 · root ≠ toplevel 은 unsupported. 닫는다.**
- [x] 7.5.3 **P1 — 이름 바뀐 고정 소스가 V1 을 다시 연다 (같은 재리뷰, 7.2.2 부터 있던 것).** 가드 7 은
      ...
      **종결(2026-09-27) [결정] 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 2 — pure rename/move 는 기존 상태 유지, 규칙 변경 없음. 닫는다.**
- [x] 7.5.6 **P1 — 열거형 호출 감사는 쉬고 있는 파일로도 꺼진다 (7.5.2.2 재리뷰 2026-09-20, 보안).** 타이밍 없이,
      ...
      **종결(2026-09-27) [개발 편의] backlog — 먼저 쟀다(`analysis/harness/756_census.py`, 2026-09-27 06:46, HEAD `a1248b98`): change 디렉터리 128 · 번들 3,084 · 번들 파일 12,337 중 UTF-8 BOM 0 · UTF-16 BOM 0 · NUL 0 · UTF-8 아님 0, 감사가 켜진 change 1(a112 — 계측기 대조), BOM 을 지우고 좌표 없는 행을 뺀 관대한 판과 감사 스위치가 갈리는 change **0**. 실제 오판정 사례 없음 — 사용자 결정 4 대로 backlog.**
- [x] 7.5.7 **P1 — `revision: base` 는 파일 수준 소스 해시에 안 묶인다 (7.5.2.4 재리뷰 2026-09-21, 보안 · 내가 정정).**
      ...
      **종결(2026-09-27) [결정] 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 5 — source hash 를 의무로 하지 않고 commit + path 만. 결속을 더하지 않는다(기존 18 건 거절 없음). 닫는다.**
- [x] 7.5.8 **P0 — 판정 입력의 일부를 자식 프로세스가 읽어 원장 밖에 있다 (7.5.2.3 재리뷰 2026-09-21, 출처 둘 + 나).**
      ...
      **종결(2026-09-27) [감사·재현성] backlog — 판정 중 자식 프로세스가 읽는 것이 바뀌는 모양(동시 변경)은 원장 밖이다. 7.5.25 가 워킹트리 Go 바이트를 스냅숏으로 좁혔고 추적 목록은 원장에 든다(7.5.9). 정상 실행을 틀리게 하지 않는다.**
...
- [x] 7.5.17 **P2 — 저자가 판정을 **판정 줄 0개로** 무한히 늘릴 수 있다 (같은 재리뷰, 보안).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — 비용 절반은 7.5.9 로 닫혔고(재측정 영수증 위), 남은 것(해소 못 한 좌표가 판정 줄을 안 냄 — 오늘의 정책 · `gate.sh` 시한)은 편의.**
...
- [x] 7.5.19 **P3 — `AA6`(재확인이 HEAD 를 먼저 묻지 않는다)이 구조로만 못 박혔다 (같은 재리뷰, 시험품질).**
      순서는 **관찰 가능**하다 — 원장 경로가 바뀌고 이웃 커밋이 같이 서면 HEAD 문장 대 경로 문장으로 갈린다.
      기록 경로는 이미 행동으로 못 박혀 있고 판정 경로만 없다.
      **종결(2026-09-27) [개발 편의 · 시험품질] backlog.**
- [x] 7.5.20 **P3 — `READ_CAP` 값에 시험이 없다 (같은 재리뷰, 시험품질).** `16 << 20` → `1 << 20` 으로 바꿔도 전부 초록.
      ...
      **종결(2026-09-27) [개발 편의 · 시험품질] backlog.**
- [x] 7.5.21 **P3 — `main()` 의 git 호출이 결함 경계 **밖**이고, 잔챙이 셋 (같은 재리뷰, 보안 · 정확성).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — git 이 조언 줄의 커밋 수 세기에서 멎을 때 판정 줄 대신 traceback 이 나간다(rc 는 0 이 아니다 — 막는 쪽). 출력 품질.**
- [x] 7.5.36 **P2 — 재확인 때만 추적 목록을 못 물으면 사유 이름이 틀린다 (7.5.9 로트 독립 적대 리뷰 P2-1, 2026-09-26).**
      ...
      **닫음(2026-09-27).** 재확인이 추적 목록을 **못 물으면** `RuntimeError("cannot re-read the tracked file list to confirm the verdict: …")` — 판정은
      ...
      again`. 변이 MW3 CAUGHT.
- [x] 7.5.37 **P2 — 이름 인용은 거절, 좌표 인용은 건너뜀 — 그리고 작성 중인 미추적 시험 파일 (같은 리뷰 P2-3).**
      ...
      **종결(2026-09-27) [개발 편의] backlog — 작성 중인 미추적 시험 파일은 `git add` 로 풀린다(거절 문장이 "in any tracked file" 이라고 말한다). 비대칭은 오늘의 정책.**
...
- [x] 7.5.39 **P2 — AA8b 생존: "HEAD moved" 판정이 두 집에 산다 (같은 리뷰 P2-6).** `_head_moved` 를 `if False` 로 죽여도
      ...
      **종결(2026-09-27) [개발 편의 · 시험품질] backlog — 판정은 맞다(두 자리가 같은 답을 낸다).**
- [x] 7.5.40 **P3 — `_recording_refusal` 의 `git diff --quiet` 에 `SNAPSHOT_PINS` 가 없다 (7.5.9 로트 주장정확성 리뷰 F9 부수, 이 로트 전부터).**
      `core.fsmonitor` 에 적힌 프로그램이 기록 명령 · 조언 줄에서 뜰 수 있다. 7.5.9 보수는 새 `ls-files` 만 고정했다.
      **종결(2026-09-27) [개발 편의] backlog — 기록 명령 · 조언 줄의 `git diff --quiet` 에서 저장소가 설정한 fsmonitor 가 뜰 수 있다. 판정 경로(`check`)는 고정됐다.**
- [x] 7.5.41 **P3 — `_safe_changed_go_paths` 의 `\n` · `\r` · `\t` 거절을 받을지 (7.5.13 에서 연다, 2026-09-27).**
      ...
      **종결(2026-09-27) [결정 · 극단 edge] 사용자 결정 2026-09-27 — review.md `## 사용자 결정 기록 (2026-09-27)` 7 — 제어 문자 파일명은 unsupported, 처리 로직을 만들지 않는다. `_safe_changed_go_paths` 가드 주석에 적었다. `\n` · `\r` · `\t` 거절은 그대로. 닫는다.**
...
- [x] 7.5.43 **P3 — `log.showSignature=true` 와 서명 커밋이면 자기 수리 목록이 멈춘다 (같은 리뷰, 이 로트 전부터).**
      ...
      **종결(2026-09-27) [극단 edge] unsupported — `log.showSignature=true` + 서명 커밋이면 멈춘다(막는 쪽, 결함 문장).**
