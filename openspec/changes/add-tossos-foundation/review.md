...
| # | 결정 | 반영 위치 | 원칙 |
|---|------|-----------|------|
| A1 | ~~go-live 승격 사다리를 제품 불변조건화~~ → **사용자 결정으로 기각(2026-07-26, U2)**: 단독 사용자 제품, 승격 단계 없이 실전 직행. 대체 안전장치 = Guardian 한도 + kill switch + 기동 인터록(A2). T1.14는 주문 경로 검증(1회성)으로, tracer slice는 실전 소액으로 재정의 | ROADMAP 원칙7 개정 | 사용자 |
...
1. **U1 Go module 경로**: 유지(기본 채택) vs 조기 rename — 이견 없으면 유지로 진행
2. ~~**U2 go-live 승격 사다리(A1)**~~ → **결정됨(2026-07-26)**: 승격 단계 불필요, 실전 직행. 문서 반영 완료
3. **U3 운영 파라미터 수치**: 미확정 시 StockOS small_live 보수 기본값으로 시작 (기본 채택)
4. **U4 Phase 5 분할(A18)**: 5a 운영 콘솔 우선 (기본 채택, 이견 시 조정)
5. **U5 origin 비공개 저장소**: 생성 시점과 이름 (사용자 GitHub 작업 — push 필요 시점까지 보류 가능)
6. **U6 markout 윈도우**: 5/15/30분 기본 채택 (Phase 3 스펙에서 변경 가능)
