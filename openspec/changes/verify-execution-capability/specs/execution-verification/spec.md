...
### Requirement: 멱등키 실동작 검증
공식 API는 주문·조건주문 생성에 클라이언트 제공 멱등키를 문서화하고 있으나 문서는 실측이 아니므로, 자동 해소 절차가 이에 의존하기 전에 실계좌로 확인되어야 한다(SHALL). 확인 항목: 동일 키·동일 본문 재요청이 이전 결과를 그대로 반환하는지(새 주문 생성 여부), 유효 창의 실제 길이, 키의 스코프(계좌 단위 여부), 본문이 다를 때의 충돌 오류 동작, 조건주문 생성에서도 동일한지. 결과는 attestation 속성으로 기록되며, 검증되지 않은 상태에서 멱등 재생을 해소 절차로 사용해서는 안 된다(SHALL NOT — 조회 기반 폴백만 사용한다).

#### Scenario: 재생이 새 주문을 만드는 경우
- **WHEN** 동일 키·동일 본문 재요청이 이전 결과가 아니라 새 주문을 생성하면
- **THEN** 멱등 재생 경로가 비활성으로 기록되고 해소는 조회 기반 절차만 사용한다

#### Scenario: 검증 완료 후 해소 절차
- **WHEN** 멱등키 동작이 문서대로임이 확인되어 attestation에 기록되면
- **THEN** 응답 유실 해소가 재요청으로 식별자를 회수하는 경로를 사용할 수 있다
