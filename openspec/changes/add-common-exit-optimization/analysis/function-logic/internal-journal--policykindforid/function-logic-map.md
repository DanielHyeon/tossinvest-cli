# Function Logic Map: `policyKindForID`

- Source: `internal/journal/adoption.go`
- Qualified function: `policyKindForID`
...
|---|---|---|---|---|
| B1 | `if` at `internal/journal/adoption.go:469` — if strings.TrimSpace(policyID) == "" { | only mutations visible in this branch and its callees | existing return/error contract | `TestLadderExitStateSnapshotsItsPolicyID`; `TestAdoptionPolicySnapshotSurvivesUntilExitStateRecovery`; journal adoption/exit-state tests |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `strings.TrimSpace` | preserve the function's validation, persistence, routing, or evaluation contract | errors remain fail-closed; no retry or authority expansion is introduced here | CodeGraph + `ast.json` |

## State mutations and fallbacks

- AST records 0 assignment(s) and 2 return(s); branch rows bind every control-flow site to regression evidence.
