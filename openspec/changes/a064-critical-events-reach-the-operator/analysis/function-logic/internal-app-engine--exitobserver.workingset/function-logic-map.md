...
delivery can wait." **a064가 publisher를 실제로 배선하므로 이 순서가 처음으로 의미를
갖게 된다.** 순서를 바꾸지 않는 것이 이 change의 §0.3 조건이다.

**a064가 바꾸는 것**: 격리를 만드는 두 경로(B11 `stored_snapshot_corrupt`, B18
`legacy_policy_identity_unknown`)에서 격리 생성 이벤트를 발행한다. 발행은 격리 행이
`refused`에 append된 **뒤**, 즉 분류가 끝난 뒤에 한다.

**a064가 바꾸지 않는 것**: 격리 조건, 분류 규칙, 반환 순서, `cycle.Err` 설정 위치.
...
- §0.3: 새 호출은 `o.alert` → `Notifier.Notify`이고, 격리된 포지션에 대해서만 실행된다.
  ...
  append 순서 결정이 지키려는 것이 정확히 이것이고 a064는 그 순서를 유지한다.
