...
**a064가 바꾸는 것**: B9 안에서 `ErrExitSnapshotQuarantined`를 알아보고, 활성 격리를
되읽어 생성 이벤트를 발행한 뒤 **error를 그대로 반환한다.**

**a064가 바꾸지 않는 것**: 반환 여부와 반환값. 격리는 여전히 사이클 실패이고, 판정은
...
- 원장 write는 `RecordExitJudgementResult` 한 번뿐이고 a064는 그것을 편집하지 않는다.
