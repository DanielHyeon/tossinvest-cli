# Function Logic Map: `TestTheMarketNeverComingIsNotAFailure`

- Source: `internal/verifylive/steps_trigger_test.go:236-254`
- Qualified function: `TestTheMarketNeverComingIsNotAFailure`
- Revision: `current`
...
|---|---|---|---|---|
| B1 | `if` at `internal/verifylive/steps_trigger_test.go:240` — `if got := h.verdict(StepConditionalTrigger); got != VerdictSkipped {` | Preserve source ordering; missing causal authority must HOLD | `TestTheMarketNeverComingIsNotAFailure` |
| B2 | `if` at `internal/verifylive/steps_trigger_test.go:244` — `if !strings.Contains(e.Reason, "INCONCLUSIVE") {` | Preserve source ordering; missing causal authority must HOLD | `TestTheMarketNeverComingIsNotAFailure` |
| B3 | `if` at `internal/verifylive/steps_trigger_test.go:247` — `if !observationEquals(t, h.entries(), StepConditionalTrigger,` | Preserve source ordering; missing causal authority must HOLD | `TestTheMarketNeverComingIsNotAFailure` |
| B4 | `if` at `internal/verifylive/steps_trigger_test.go:251` — `if out := Outstanding(h.entries()); len(out) != 0 {` | Preserve source ordering; missing causal authority must HOLD | `TestTheMarketNeverComingIsNotAFailure` |
