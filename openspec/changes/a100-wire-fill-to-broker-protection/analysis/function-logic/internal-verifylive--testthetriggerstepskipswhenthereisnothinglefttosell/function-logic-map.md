# Function Logic Map: `TestTheTriggerStepSkipsWhenThereIsNothingLeftToSell`

- Source: `internal/verifylive/steps_trigger_test.go:387-401`
- Qualified function: `TestTheTriggerStepSkipsWhenThereIsNothingLeftToSell`
- Revision: `current`
...
|---|---|---|---|---|
| B1 | `if` at `internal/verifylive/steps_trigger_test.go:395` — `if got := h.verdict(StepConditionalTrigger); got != VerdictSkipped {` | Preserve source ordering; missing causal authority must HOLD | `TestTheTriggerStepSkipsWhenThereIsNothingLeftToSell` |
| B2 | `if` at `internal/verifylive/steps_trigger_test.go:398` — `if n := h.broker.countRequests("POST /conditional-orders 005930 key=TRIGGER-"); n != 0 {` | Preserve source ordering; missing causal authority must HOLD | `TestTheTriggerStepSkipsWhenThereIsNothingLeftToSell` |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `withBook` | Source dependency | Propagate errors; M0 critical gaps are irreversible | AST |
| `withHolding` | Source dependency | Propagate errors; M0 critical gaps are irreversible | AST |
| `newFakeBroker` | Source dependency | Propagate errors; M0 critical gaps are irreversible | AST |
| `newHarness` | Source dependency | Propagate errors; M0 critical gaps are irreversible | AST |
| `alwaysConfirm` | Source dependency | Propagate errors; M0 critical gaps are irreversible | AST |
