...
- **`tossctl engine run`** 신설: config 로드 → journal open(RW) → 브로커 조립(구조적 official-only 유지) → **기동 인터록 검증**(기존 engine-safety 요구사항의 소비자 — 게이트 OFF 또는 조항 미충족이면 미충족 항목을 열거하며 기동 거부, fail-closed) → 루프 기동(reconcile driver 60s·exit observer 5s) → graceful shutdown.
- **루프 전멸 원칙**: 어느 루프든 오류로 종료하면 전체 엔진이 정지하고 critical 알림을 발송한다 — 포지션을 들고 있는 상태에서 exit 관측만 조용히 죽는 부분 생존은 보호 결손이므로 금지(§0.3 정합).
...
- **무인 운용 표면**(사용자 결정 2026-07-27 — 터미널 조작 제거 원칙): ① 부팅 자동 시작 — soak-autostart 패턴의 엔진 자동 기동(게이트 OFF·인터록 미충족이면 조용히 대기·재시도, 켜지는 순간은 §0.7 승인이 이미 끝난 게이트 설정이 결정), 크래시 시 제한적 자동 재시작(횟수 상한·critical 알림 유지 — 2c의 브로커측 보호가 엔진 사망 중 포지션을 지키는 전제와 정합) ② 콘솔 대시보드에 엔진 상태 표시 + [엔진 시작]/[엔진 정지] 버튼(세션+CSRF — soak 재시작과 동일 모델). **게이트 flip은 여전히 콘솔 밖**(§0.7 사람 승인 절차 — 버튼은 이미 승인된 설정의 프로세스 기동/정지만 한다).
...
- §0 검토: 새 주문 경로 0, 기동은 인터록 뒤에만, 부분 생존 금지는 보호 강화 방향, verify 병행 거부는 예산 보호
