...
- Source: `internal/strategyprojectionrpc/transport_unix.go` (466-528) — revision `current`
- Fix 라운드가 이 함수를 stage+rename으로 다시 썼다(design D1-2, A1 F2).
- gstack 라운드가 임시 **이름**을 `os.CreateTemp` 에서 `stagingPath` 로 옮겼다.
  이유는 길이다: `CreateTemp` 는 임의 자릿수 숫자를 붙여 이름 길이를 계약으로
  잡을 수 없는데, 같은 규칙을 쓰는 socket 쪽에서는 그 길이가 bind 가능성
  자체를 정한다(sun_path 107자). 분기가 12 → 13 으로 하나 늘었다(B2·B3 이
  옛 B2 를 대체한다).

| Branch | Scenario | Test | RED observed | GREEN observed |
|---|---|---|---|---|
| B1 | JSON marshal 실패 | 없음 — 아래 「실패 분기를 재지 않은 이유」 | no | no |
| B2 | 임시 **이름** 생성 실패(난수 읽기) | 없음 | no | no |
| B3 | 임시 파일 생성 실패(`O_EXCL` 충돌 포함) | 없음 | no | no |
| B4 | 임시 파일 chmod 실패 | 없음 | no | no |
| B5 | 임시 파일 stat 실패 | 없음 | no | no |
| B6 | 본문 write 실패 | 없음 | no | no |
| B7 | sync 실패 | 없음 | no | no |
| B8 | close 실패 | 없음 | no | no |
| B9 | 닫힌 임시 파일이 바뀌었다 | 없음 | no | no |
| B10 | rename 실패 | 없음 | no | no |
| B11 | 발행된 파일이 임시 파일이 아니다 | 없음 | no | no |
| B12 | 디렉터리 열기 실패 | 없음 | no | no |
| B13 | 디렉터리 sync 실패 | 없음 | no | no |

## 이 함수가 재는 것은 분기가 아니라 성질이다

열세 분기는 전부 **실패** 분기이고, 성공 경로에는 분기가 없다. 그래서 표의 열세 줄은
...
B1~B13은 파일시스템 오류(ENOSPC·EIO·권한)를 주입해야 닿는다. 주입 seam을 만들려면
