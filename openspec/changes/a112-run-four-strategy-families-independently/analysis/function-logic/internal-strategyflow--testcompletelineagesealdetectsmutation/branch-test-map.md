# Branch Test Map: `TestCompleteLineageSealDetectsMutation`
...
|---|---|---|---|
| B1 | if at 167:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 171:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
