...
- Source: `internal/app/engine/strategy_entry_supervisor.go` (377-420)
...
(`!p.snapshot.Ready`, `len(p.entries) != 1`)을 지우고 `p.dispatchHandoff()` 하나로 바꿨다.
다른 권한(schedule/candidate/route/fx/risk/account)의 준비 상태 검사는 그대로다.
...
Exact AST return positions: 384:3, 399:3, 403:3, 406:3, 409:3, 415:3, 417:2.
...
| `p.dispatchHandoff` | 396:13 |
| `handoff.Admitted` | 398:4 |
...
승격 조건이 **완화되지 않았다.** handoff 는 이전 두 조각이 통과시키던 것과 정확히 같은 집합을
