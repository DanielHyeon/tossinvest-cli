# Branch Test Map: `TestWrongMarketLaneInputAndForgedAcceptedLineageFailClosed`
...
|---|---|---|---|
| B1 | if at 150:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 155:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
