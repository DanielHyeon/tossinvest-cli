...
- Exact AST return nodes: `544:3, 552:3, 555:3, 559:2, 560:3`.

| Branch | AST kind | Source location | Test disposition |
|---|---|---|---|
| B1 | if— `c` 또는 시계가 없다 | 543:2 | arm entered 1x (engine tagged suite); arm entered 1x (engine untagged suite); `TestARefreshWithoutAClockRefusesBeforeItCanMintAWave` |
| B2 | if— 창 안의 캐시가 있다 | 551:2 | arm entered 1x (engine tagged suite); arm entered 1x (engine untagged suite); `TestTheMarketThatLeadsAWaveAlwaysPublishesIt` |
| B3 | if— 지도자가 아니다 (파도에 합류) | 554:2 | arm entered 5x (engine tagged suite); arm entered 5x (engine untagged suite); `TestAFailedWaveCarriesNoAssemblyForEitherMarket`, `TestAFailedWaveReachesEveryMarketAndIsNeverCached`, `TestAMarketWaitingOnTheWaveLeavesWhenItsOwnCycleIsCancelled`, `TestAPanickingWaveNeverStrandsTheMarketsWaitingOnIt`, `TestTwoMarketsRideOneAuthorityWaveInsteadOfTakingTurns` |

## Calls and live bindings

| Callee expression | Source location | Note |
|---|---|---|
| errors.New | 544:45 | B1 의 거절 |
| UTC | 546:9 | 창을 재는 시각을 UTC 로 고정한다 |
| clk.Now | 546:9 | 창을 재는 유일한 시각. 잠금 **밖**에서 읽는다 |
| c.joinStrategyRefreshWave | 550:26 | 잠금 안에서 하는 일의 전부 |
| awaitStrategyRefreshWave | 555:10 | 채널에서 기다린다 — `ctx` 를 함께 본다 |
| c.collectStrategyRefreshWave | 559:9 | 지도자의 자리. 잠금 밖 |
| c.NewPairedStrategyEntryProductionAssembly | 560:10 | 원격 파도 본체. 클로저 안이므로 `collect` 인자로 넘어간다 |
