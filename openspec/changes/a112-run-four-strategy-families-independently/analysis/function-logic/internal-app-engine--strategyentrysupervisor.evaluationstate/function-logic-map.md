...
|---|---|---|---|---|
| B1 (`975:2`) | 종료 중이거나 nil 이거나 **잠겼거나** 사이클이 없거나 (dormant 이고 갱신자도 아님) | 없음 (RLock 만) | `(false, false)` (`895:3`) | `TestALatchedMarketSkipsTheTriggersAlreadySittingInItsQueue` |
| B2 (`979:2`) | 권한 갱신 worker | 없음 | `(true, false)` (`898:3`) | `TestTheOnlyWorkerProductionActuallyRunsSwallowsEveryCycleError` |
...
|---|---|---|
| `s.mu.RLock` | 973:2 | 읽기 잠금 — 이 함수는 아무것도 바꾸지 않는다 |
| `s.mu.RUnlock` | 974:8 | `defer` |
| `Before` | 982:11 | 권한 신선도 비교 |
| `s.clk.Now` | 982:11 | 주입 시계 |

Exact AST return positions: 977:3, 980:3, 983:2.
