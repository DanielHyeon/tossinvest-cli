# Branch Test Map: `TestEvaluateRejectsAcceptedLaneWithoutExactExecutionTerms`
...
|---|---|---|---|
| B1 | if at 70:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 83:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
