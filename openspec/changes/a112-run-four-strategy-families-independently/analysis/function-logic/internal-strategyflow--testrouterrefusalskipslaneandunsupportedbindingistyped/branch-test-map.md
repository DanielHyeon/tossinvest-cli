# Branch Test Map: `TestRouterRefusalSkipsLaneAndUnsupportedBindingIsTyped`
...
|---|---|---|---|
| B1 | if at 98:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 107:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
