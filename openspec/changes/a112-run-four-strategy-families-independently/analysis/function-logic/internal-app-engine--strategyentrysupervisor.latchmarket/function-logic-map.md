...
- Source SHA-256: `4e457c677157b2f8c73f813f8250575657b6beedddc1ad467db209a35579986d`
...
|---|---|---|---|---|
| B1 (`917:2`) | 실패 문구가 빈 문자열 | 지역 `reason` 만 | — | `TestMarketFailureEmitsExactIrreversibleFaultAndKeepsPeerSafetyAlive`(음의 갈래) |
| B2 (`921:2`) | `abnormal` | 지역 `refusal` 만 | — | `TestPairedMarketAbnormalReturnSchedulesOnlyLocalBoundedRestartAndKeepsEverySafetyLoopAlive` |
| B3 (`924:2`) | 실패가 `ErrStrategyAuthorityExpired` | 지역 `refusal` 만 | — | `TestExpiredAuthorityLatchesBeforeEvaluation` |
| B4 (`928:2`) | 관측 시각이 0 | **없음 — 잠금 전에 반환한다** | `errors.New("strategy fault observation time is unavailable")` (`929:3`) | `TestTheFourEscalationsThatStopTheEngine…`/"관측 시각이 없으면…" |
| B5 (`932:2`) | `worker.latchRevision == math.MaxUint64` | 잠금 mutex 를 풀고 **아무것도 바꾸지 않는다** | `errors.New("strategy latch revision exhausted")` (`934:3`) | 같은 시험의 "latch revision 이 소진되면…"·"권한 만료의 잠금도…" |
| B6 (`938:2`) | 첫 refusal 이 비어 있음 | `worker.firstRefusal` | — | `TestMarketRestartAttemptAndDeadlineSaturateWithoutOverwritingFirstTypedRefusal` |
| B7 (`941:2`) | 첫 실패 문구가 비어 있음 | `firstFailure`·`firstAbnormal`·`latchID`·`latchRevision++` | — | 같은 시험 |
| B8 (`947:2`) | 재시작 시도 수가 상한 미만 | `restartAttempt++` | — | 같은 시험(포화 갈래 포함) |
| B9-a (`962:2`) | fault 를 스트림에 건넴 | 없음 | `fault.RestartNotBefore, nil` (`963:3`) | `TestEveryWorkerCanHandOffItsFaultWithoutAnybodyDraining` |
| B9-b (`964:2`) | 스트림 포화 | 없음 — **잠금은 이미 일어났다** | `errors.New("strategy fault handoff saturated …")` (`965:3`) | **없음 — 오늘은 도달 불가(아래)** |
...
|---|---|---|
| `strings.TrimSpace` | 916:12 | 실패 문구 정규화 |
| `failure.Error` | 916:30 | 잠금 이유의 원문 |
| `errors.Is` | 924:5 | 권한 만료를 별도 refusal 로 분류 |
| `s.clk.Now` | 927:16 | **관측 시각.** 주입 시계이지 `time.Now` 가 아니다 |
| `observedAt.IsZero` | 928:5 | B4 — 시각 없이 잠금을 기록하지 않는다 |
| `errors.New` | 929:23 | B4 의 오류 |
| `s.mu.Lock` | 931:2 | 상태 변경 구간 시작 |
| `s.mu.Unlock` | 933:3 | B5 의 조기 반환이 잠금을 푼다 |
| `errors.New` | 934:23 | B5 의 오류 |
| `fmt.Sprintf` | 944:20 | `latchID` — market·generation·revision+1 |
| `strategyRestartBackoff` | 950:18 | 5s 계단, 30s 상한 |
| `strategyRestartNotBefore` | 951:28 | 절대 기한. 9999 년으로 포화 |
| `observedAt.UTC` | 957:119 | fault 의 관측 시각 |
| `s.mu.Unlock` | 960:2 | 상태 변경 구간 끝 |
| `errors.New` | 965:23 | B9-b 의 오류 |

Exact AST return positions: 929:3, 934:3, 963:3, 965:3

## State mutations and fallbacks

- 상태 변경은 전부 `931:2`–`960:2` 의 한 임계 구간 안에 있다: `effective=false`,
...
- **인용해 가는 사실 하나 (측정):** B9-b 는 오늘 **도달할 수 없다.** 용량이 2 이고,
  잠긴 시장은 `evaluationState`(`857:2`)가 거부하므로 시장당 잠금은 한 번이며,
