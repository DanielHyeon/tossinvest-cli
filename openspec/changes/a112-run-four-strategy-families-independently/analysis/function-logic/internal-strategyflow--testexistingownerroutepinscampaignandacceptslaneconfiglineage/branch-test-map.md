# Branch Test Map: `TestExistingOwnerRoutePinsCampaignAndAcceptsLaneConfigLineage`
...
|---|---|---|---|
| B1 | if at 186:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 196:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
