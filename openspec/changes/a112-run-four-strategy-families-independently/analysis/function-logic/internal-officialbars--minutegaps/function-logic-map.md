...
- Source range: `435:1`–`456:2` (ast.json `start`/`end`)
- AST evidence: `ast.json` generated 2026-08-17; branches 5, returns 1, calls 11, assignments 8, defers 0, go statements 0.
...
Exact AST return node: `455`.

| Branch | AST kind | Source location | Meaning (one short clause) | Test disposition |
|---|---|---|---|---|
| B1 | for | 437:2 | walk consecutive observed pairs newest→oldest, taking the half-open span `(older+1m, newer]` | `TestPollGapsAreClampedToTheRegularWindow` (3 gaps), `TestPollStoresOnlyRegularSessionBarsButUsesTheOthersAsSuccessors` (one 388-minute gap) |
| B2 | if | 440:3 | clamp the start up to the regular open (a pre-session hole is reported only from the open) | `TestPollGapsAreClampedToTheRegularWindow` (`Gaps[2]` starts exactly at the open, 1 minute) |
| B3 | if | 443:3 | clamp the end down to the regular close (an after-hours hole stops at the close) | `TestPollGapsAreClampedToTheRegularWindow` (`Gaps[0]` = 386 minutes, `To` = close − 1 min) |
| B4 | if | 446:3 | after clamping the window is empty → report nothing for this pair | every adjacent-minute pair, e.g. `TestPollCrawlsPagesInTheMeasuredShape` and `TestPollNeverAdmitsTheNewestObservedBar` (no gaps reported) |
| B5 | if | 450:3 | a positive but sub-minute span → report nothing | not-applicable: ruling 26 forces every bar instant onto a minute boundary and the calendar's window edges come from `AdaptOfficialCalendar` at whole minutes, so after B4 the span is always at least one whole minute; defensive |

## Calls and live bindings

| Callee expression | Source location | Evidence |
|---|---|---|
| `older.Add(time.Minute)` | 439 | the first possibly-missing minute is the one after the older bar |
| `start.Before(regularOpen)`, `end.After(regularClose)` | 440, 443 | the two clamps; `TestPollGapsAreClampedToTheRegularWindow` asserts both edges |
| `start.Before(end)` | 446 | emptiness test after clamping |
| `int(end.Sub(start) / time.Minute)` | 449 | whole-minute count |
| `append(gaps, MinuteGap{…})`, `start.UTC()`, `end.Add(-time.Minute).UTC()` | 453 | inclusive `To`, UTC normalisation |
