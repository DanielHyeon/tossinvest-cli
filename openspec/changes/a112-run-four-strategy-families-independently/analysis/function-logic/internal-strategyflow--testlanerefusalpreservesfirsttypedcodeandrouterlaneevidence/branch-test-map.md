# Branch Test Map: `TestLaneRefusalPreservesFirstTypedCodeAndRouterLaneEvidence`
...
|---|---|---|---|
| B1 | if at 122:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B2 | if at 125:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
| B3 | if at 129:2 | guard arm — reached only when the assertion fails, so a passing run must not enter it | passing run: not entered |
