...
- Source range: `396:1`–`421:2` (ast.json `start`/`end`)
- AST evidence: `ast.json` regenerated 2026-08-18 against the decision-30 sources; branches 6, returns 7, calls 16, assignments 3, defers 0, go statements 0.
...
- **Decision 30 (2026-08-18), documentation-only.** `nextBefore` and the candle instants it is compared against are all bar **close** labels (US probe 03:29 KST; review.md), so the loop guard compares like with like and is unchanged. The producer keeps its own D4 guard in the same label space and never converts a cursor — only bars are converted.
...
Exact AST return nodes: `398`, `403`, `406`, `410`, `414`, `417`, `420`.

| Branch | AST kind | Source location | Meaning (one short clause) | Test disposition |
|---|---|---|---|---|
| B1 | if | 397:2 | the `nextBefore` key is absent → `CURSOR_INVALID` ("absent is not terminal") | `TestStrictMinuteCandlesRefusesMalformedBodies` (subtest `cursor replaced by another key`, the two-key body that the key-count rule cannot catch) |
| B2 | if | 402:2 | the value is `null` → terminal page, empty cursor | `TestStrictMinuteCandlesAcceptsTheKoreanMarket`, `TestStrictMinuteCandlesIgnoresUnknownEnvelopeKeys`, `TestStrictMinuteCandlesAcceptsNestingUpToTheBound` (all assert `page.Terminal`) |
| B3 | if | 405:2 | the value is neither `null` nor a JSON string → `CURSOR_INVALID` | `TestStrictMinuteCandlesRefusesMalformedBodies` (subtests `cursor is a number`, `cursor is an object`, `cursor is an array`, `cursor is a boolean`) |
| B4 | if | 409:2 | the string is empty (or not extractable) → `CURSOR_INVALID` | `TestStrictMinuteCandlesRefusesMalformedBodies` (subtest `cursor is an empty string`) |
| B5 | if | 413:2 | the cursor fails the timestamp grammar or names an instant that does not exist → `CURSOR_INVALID` | `TestStrictMinuteCandlesRefusesMalformedBodies` (subtest `cursor breaks the timestamp grammar`, a `Z` offset) |
| B6 | if | 416:2 | the cursor is not strictly older than this page's oldest bar → `CURSOR_INVALID` | taken: `TestStrictMinuteCandlesRefusesMalformedBodies` (subtests `cursor is not older than the oldest bar`, `cursor is newer than the oldest bar`); untaken with an empty page: `TestStrictMinuteCandlesAcceptsAnEmptyPageThatStillCarriesACursor`; sub-minute cursor accepted: `TestStrictMinuteCandlesAcceptsASubMinuteCursor` |
...
| `strictMinuteInstant(value)` | 412 | the same grammar the request's `before` had to satisfy |
| `cursor.Before(instants[len(instants)-1])` | 416 | the loop guard against the page's oldest bar |
| `strictMinuteRefuse(StrictReasonCursor, …)` ×5 | 398, 406, 410, 414, 417 | one reason label for every cursor break, asserted by the reader table |
