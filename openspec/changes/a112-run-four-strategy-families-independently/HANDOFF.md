...
한 줄(2026-09-03 세 번째 갱신): 5 절에서 남은 것은 **5.1.2.2 · 5.2.2 · 5.3.3 · 5.6.2** 다.
...
| 이 커밋 | 5.2.1 랜딩 — 원격 권한 수집이 공유 assembly mutex 밖으로 나갔다(single-flight 파도). 두 시장이 한 파도를 타고, 기다리는 쪽이 `ctx` 를 본다. `internal/app/engine` 의 동시성 시험이 처음으로 `-race` 아래 돈다 |
...
3. 그다음 열려 있는 태스크로 간다. 5 절에서 남은 것은
   **5.1.2.2 · 5.2.2 · 5.3.3 · 5.6.2**, 그다음이 6 절이다.
