...
한 줄(2026-09-03 네 번째 갱신): 5 절에서 남은 것은 **5.1.2.2 · 5.2.2 · 5.6.2** 다.
...
| 이 커밋 | 5.3.3 랜딩 — 레인 잠금이 원장(schema **v32**)에 남아 재시작을 견딘다. 복구는 **엄격히 더 큰 서명 활성화 세대**가 있어야 하고 그 판정은 SQLite 트리거가 한다. 여덟은 여전히 DORMANT 라 **오늘 쓰이는 행은 0** |
...
3. 그다음 열려 있는 태스크로 간다. 5 절에서 남은 것은
   **5.1.2.2 · 5.2.2 · 5.6.2**, 그다음이 6 절이다.
