...
## §B 생존 뮤테이션 (6건) — 사유와 함께 선언

| # | 뮤테이션 | 왜 죽일 수 없었나 |
| --- | --- | --- |
| M1a | 낯선 엔트리를 `seen[name] = true`로 (거부 대신 무시) | **등가에 가깝다.** 낯선 엔트리를 무시해도 제거 목록에 들어가지 않으므로 마지막 rmdir이 ENOTEMPTY로 실패하고, 기동은 여전히 거부되며 그 파일도 그대로 남는다. 즉 "낯선 엔트리 거부"는 **두 겹**이다 — 명시적 거부와 rmdir 실패. 위험한 변형은 M1b(우리 잔재로 친다)이고 그것은 죽었다. |
...
| M22 | staged listen의 `SetUnlinkOnClose(false)`를 지운다 | **경합이다.** late-unlink는 `Shutdown`이 `Serve`의 listener 등록을 앞지를 때만 나타나고 a108도 300라운드 중 3회로 관측했다. 그리고 이 change에서는 방어가 두 겹이라(listener가 기억하는 이름은 이미 rename으로 사라진 임시 이름) 이 한 겹을 지워도 관측 가능한 피해가 없다. |
| M23 | 발행을 **최종 경로 bind**로 되돌린다(stage+rename 폐기) | **crash 창이다.** 이 뮤테이션이 되살리는 것은 "listen과 chmod 사이에서 죽으면 최종 이름에 pre-chmod socket이 남는다"인데, 테스트는 그 두 줄 사이에서 프로세스를 죽일 수 없다. §1.1은 그 창의 **결과**(잔재에서의 기동)를 잡지 회수 방식으로 잡는 것이라 stage+rename 없이도 통과한다. 근거는 테스트가 아니라 수명주기 논증이다(design D1). |
| M26 | `AlertControlServer.Close`의 listener 직접 닫기를 지운다 | M22와 같은 경합. Close 계약(세 경로 부재)은 그대로 통과한다 — 그것을 재는 테스트는 있고, 늦은 unlink를 재는 테스트는 없다. |
...
생존 6건 중 **동작을 바꾸는 것은 M12 하나뿐**이고(나머지는 등가이거나 경합 창),
그 하나는 비root 환경의 측정 한계다.
...
- 적용 31건 = **사망 25 · 생존 6**(등가 2 · 경합 3 · 측정 불가 1)
- 뮤테이션이 **새 핀 3개를 열었다**(hard link · 디렉터리 위생 · descriptor 모양).
  §1.5 경계 테스트만으로는 A4의 네 뮤테이션이 전부 살아남았다.
- 회수의 모든 분기(분류 5 · probe 4 · 완화 4 · 검증 5 · 제거·위생 6)에 죽는 뮤테이션이
  하나 이상 있다. 예외는 §B에 사유와 함께 선언했다.
