...
| B11 | `if err := r.recorder.Append(entry); err != nil {` (internal/verifylive/runner.go:320) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
| B12 | `if sr.verdict == VerdictAwaitingRestart {` (internal/verifylive/runner.go:329) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
| B13 | `if errors.Is(sr.abort, ErrNotATerminal) {` (internal/verifylive/runner.go:335) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
| B14 | `if errors.Is(sr.abort, ErrOutsidePlan) {` (internal/verifylive/runner.go:341) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
| B15 | `if sr.abort != nil && errors.Is(sr.abort, context.Canceled) {` (internal/verifylive/runner.go:350) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
| B16 | `if leftovers := undeliberate(summary.Outstanding); len(leftovers) > 0 {` (internal/verifylive/runner.go:359) | internal/verifylive/cleanup_test.go, internal/console/retry_after_run_test.go 및 기존 패키지 테스트 | yes | yes |
