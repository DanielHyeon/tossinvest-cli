...
- [x] 2.4 **번들 둘을 더 만든다** (1라운드 A-T2·B-T2):
      ...
      **총 열하나가 된다**
- [ ] 2.5 **branch-test-map의 Test 칸을 전부 실측한다.** 1라운드 B-T1이
...
- [ ] 3.1 **RED 오늘** 넷(R1~R4)을 작성하고 **실패를 실제로 관측한다.** 출력을 여기 붙인다
...
- [ ] 3.4 **임차 기간을 유도식으로 정한다** (design C4). 상수 하나를 고르는 것이 아니다.
...
- [ ] 3.4b `:skew` 값을 정한다. **하한은 2초**다 — RFC3339 절삭 1초 + 여유 1초(C2).
