...
- [ ] 1.6 **성과 동결의 편입 분기**(design A7): computeTradeOutcome의 `decisionID==""` 조기 반환을 adoption_id 분기로 확장 — 매수 leg은 position_adoptions에서 합성(수량 = adoption.quantity, **기준가 = observed_price 단일** — cost_basis는 산식 제외·기록용), 매도 leg은 **명시 조인**(exit_events→mutation_attempts→fill_events CTE — 심볼 시간창 매칭 금지), realized_r 분모는 합성 initial_risk. trade_outcomes 스키마 무변경

## 2. 편입 파이프라인 [T]

- [ ] 2.1 **엔진 reconcile 구동 루프 신설**(design A6): 주기 60초, 전체 스냅샷(미체결 pagination ≤ MaxPages + holdings + 통화별 잔고 — 부분 실패 폐기) → Stabiliser(수집 2회) → 비교·fold → **Tracker.Observe 구동**(확정 하한 캡의 전제) → 편입 판정. §0.4 계상 문서화(주기당 수집 2회 × (2+통화 수)콜 + 편입 후보 심볼당 시세 1콜·MaxPages 상한). 실행 술어 `AutomationStatus.Verified`
