...
## 라운드 4 (2026-07-27 — 확인 리뷰, 판정 REVISE(minor) → 5판 반영)

라운드 3 결정 전건 정확 반영 확인(신선도·계상·기준가 단일화·괄호 위임 — 문서 5종 대조). 잔여 3건 처분:

- **P2 A2 제약 누락**: 채택 — design A2에 15s 신선도 제약 추가(DDL이 A2를 정본으로 지목하므로)
- **P2 "R 분모" 과소 서술**: 채택 — "R 산식(분자·분모 모두)에 쓰지 않는다"로 확대(아카이브되는 문언이 결정과 일치해야 함)
- **P2 flatten 청산 매도 leg 공백**: 채택 — **Manager 결정: 포함안** — 매도 leg은 전 발의자(exit 루프·flatten) 매도 fill을 발의자별 명시 참조로 수집. flatten 종결이 빈 매도 leg으로 동결되어 허구 전액 손실이 기록되는 경로 차단. 성과행 제외안은 실체결 데이터를 버리므로 기각
- **P3 배치 호출 문구**: 채택 — "후보 전체를 한 번의 배치 호출로"(심볼 fan-out 유도 방지)

## 라운드 5 (2026-07-27 — 판정 **FREEZE-READY**)

세 줄 전건 닫힘 확인(A2 신선도·"R 산식 전체"·flatten 포함 매도 leg — "빈 매도 leg 동결 금지"가 하중 조항: 귀속 공백이 잘못된 행이 아니라 행 부재로만 나타난다). 라운드 1~4 누적 P1 17건·P2 18건 전부 종결.

**구현 노트(P3, 비차단 — 구현 시 issues.md 기록)**: A7 두 bullet의 교차 케이스 — 엔진이 부분 매도(40%) 후 사용자가 잔량을 외부 처분해 수량 0이 되면, "전량 처분이 엔진에 귀속되지 않으면 성과 행 없음"을 기본값으로 한다(합성 전량 매수 leg에 부분 매도 leg을 맞추면 손익 왜곡).

**freeze 확정 — 구현 착수 승인 조건: add-operator-dashboard의 journal 조각 landed 후(design A9).**
