...
- [ ] 1.1 [T] `candidate.Row`·`Reported`의 신규 진입 사실을 3-상태로. **`unknown`이
...
- [ ] 5.2 [T] 그 파일들이 `execgw.`·`orderintent.`·`trading.`·`official.Place`를 **함께**
...
- [ ] 5.3 [T] **양성 대조군.** 주문 동사를 명명하는 것이 확실한 파일을 가리켜 검사가
  실제로 발화하는지 확인한다. 없으면 가드가 조용히 아무것도 금지하지 않는다
  (선례: `isolation_test.go:313`).
- [ ] 5.4 [T] 왜 지금 넣는지를 테스트 주석에 적는다 — 값이 들어간 뒤에 넣으면 그 사이에
  생긴 소비자가 목록의 초기값이 되어 **결정 없이 승인된다.**

## 6. 임계 단일 출처 (design D7)

- [ ] 6.1 [T] `cmd/tossctl/candidate.go:364`와 `cmd/tossctl/console.go:898`의 두
  리터럴을 하나의 생성자로 모은다.
- [ ] 6.2 [T] `cmd/tossctl` 생산 파일에 다른 `VetoThresholds{…}` 복합 리터럴이 없음을
  AST로 고정. 선례는 `candidate_review_test.go:193`.
  임계가 하나(`near_high`)뿐인 지금 하는 것이 싸다.

## 7. 150행 허구 정정 (design D9)

- [ ] 7.1 `add-candidate-discovery` design.md D8의 예시를 제자리에서 날짜를 붙여 정정.
  선례는 같은 문서의 `near_high` 부호 정정. 정규화 자체는 유지한다 — WTS 30행과 공식
  100행이 한 저장소에 섞이므로 정규화의 필요는 실재하고, 근거가 된 예시만 틀렸다.
- [ ] 7.2 코드 주석 정정. `rg -n '150' internal/candidate internal/candidatesrc`로
...
- [ ] 7.3 [T] 회귀 가드: 패널 크기를 주장하는 주석과 실제 소스 선언이 어긋나면 실패하는
  테스트. `fsguard_drift_test.go`가 파일을 소스로 읽어 대조하는 방식의 선례를 따른다.
  주석은 다시 낡는다 — 이번에는 낡으면 실패해야 한다.
- [ ] 7.4 §7의 기존 테스트 정정이 있으면 여기서 한다. **각각에 대해 판단한다**:
