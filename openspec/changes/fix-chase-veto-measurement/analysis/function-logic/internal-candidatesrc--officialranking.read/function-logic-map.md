...
- AST evidence: `ast.json` (revision `current`, L228–279, 분기 4개)
...
`whole` 인자는 `o.count == total`이다 — 소스 자신이 선언한 두 숫자의 비교이고,
지어낸 하한이 아니다(design D4).
...
| B4 | `symbol == ""` — 심볼 없는 행 | 행을 만들지 않는다. `total`에는 여전히 포함된다 | continue | **없음** (아래 주석) |
...
| `o.rememberRead(market, raw.Items, o.count == total)` | 직전 집합 취득 + 조건부 교체 | 오류 없음, 자체 mutex | ast.json calls |
