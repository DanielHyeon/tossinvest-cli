...
| B16 | 순위를 실은 행만 후보가 된다 | `TestASessionStartDoesNotStampThePanelAsSeenLate` | — (기존 동작) | yes |
| B17 | 자격 있는 읽기가 자격 없는 것을 밀어내고, 같은 자격끼리는 패널 순서 | `TestAQualifiedReadingTakesTheFirstSightingFromAPanelEarlierUnqualifiedOne` · `TestPanelOrderStillDecidesBetweenTwoReadingsOfTheSameStanding` · `TestWhenNoReadingInTheTickCanQualifyThePositionIsHeld` · `TestAQualifiedReadingIsAlsoPreferredWhenItArrivesForOnlySomeSymbols` | yes (옛 선택 규칙으로 되돌리면 2건 실패, 2026-07-28 실행) | yes |
| B18 | 아무 소스도 답하지 않음 | `TestEverySourceFailingIsAnError` · `TestAPanelWithNoSourcesInItIsStillAnError` | — (기존 동작) | yes |
| B19 | mis-wire가 '무응답'을 이긴다 | `TestARankingCannotClaimToHaveFallenBack` | — (기존 동작) | yes |
| B20 | 관측 기록 실패 | **커버 없음** | no | no |
| B21 | 심볼 정렬 | `scan_test.go` 전반 | — (기존 동작) | yes |
| B22 | 승격 pass | `scan_test.go` 전반 | — (기존 동작) | yes |
| B23 | 한 심볼의 승격 거부 | `TestOneRejectedSymbolDoesNotAbortTheMarket` | — (기존 동작) | yes |
| B24 | NoteSources 실패 | **커버 없음** | no | no |
| B25 | recordFirsts 실패 | **커버 없음** | no | no |
| B26 | coolAbsent 실패 | **커버 없음** | no | no |
| B27 | 완전한 결과 + mis-wire 오류 | **커버 없음** | no | no |

**정직한 커버리지 기록**. 이 change가 만든 분기(B11·B17)와 그것이 지나는 본문(B13)은 커버된다.
커버되지 않는 것은 전부 **기존 분기**이고, 목록으로 적어 둔다:

- B1·B2 — `Collect`의 인자 검증(빈 시장, zero instant)을 넘기는 테스트가 없다.
- B7 — 패널과 not-asked에 같은 소스가 있는 모순.
- B14 — 빈 심볼 행 skip.
- B20·B24·B25·B26 — 저장 실패 전파 네 곳.
- B27 — mis-wire와 성공 읽기가 함께 있는 fixture가 없다(B19는 전 소스 실패 쪽만 잡는다).
