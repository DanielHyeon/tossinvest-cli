...
- AST evidence: `ast.json` (revision `current`, L466–490, 분기 5개)
...
| `whole` | `w.size == len(stocks)` | `Read` | false면 기억을 남긴다 |
...
| B2 | `wtsSymbol(s) != ""` | 제외 시 집합에 넣지 않는다 | — | **없음** (아래 주석) |
| B3 | `w.seen == nil` | 맵 생성 | — | `TestTheWTSPopularityListReportsTheSameTwoFacts`(첫 읽기) |
| B4 | `whole` | `w.seen[key]` 교체 | — | `TestTheWTSMemoryCarriesTheSameTwoConditions`(1행/2행) |
