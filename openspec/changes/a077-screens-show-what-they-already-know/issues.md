...
## 배포 후 실측 (2026-08-04T04:28Z, task 6.7 부분)

병합 빌드 `8f0e48aaa1bf`를 운영에 올린 직후 `/position-management`를 GET으로만
읽었다. 아무것도 누르지 않았다.

| 항목 | 결과 |
|---|---|
| 466100이 `판정 격리`로 나오는가 | **확인.** `판정 격리 v1 · 복구 후보를 하나로 정할 수 없음 · 19시간 25분 경과 · 유지 보호선 24929`와 `이 포지션은 지금 exit 판정 대상이 아니다 — 손절도 평가되지 않는다` |
| `/position-management`에 한글 종목명 | **확인.** `kr · 042660 · 한화오션`, `kr · 466100 · 클로봇` |
| 다섯 값(익절·손절·추적 회수·기준·고점) | **측정 불가.** 열린 포지션 다섯 개가 전부 격리 상태라 명세대로 blanking 분기가 그려진다. 이 분기를 보려면 격리 해제가 선행되어야 하고 그것은 §0.7 사용자 판정이다 |

**부수 확인 — broker 호출을 하지 않는다는 증거.** 재시작 직후 첫 요청에서는 종목명이
비어 있었고, `/positions`를 한 번 읽은 뒤 다시 요청하니 이름이 나타났다. 즉 이름은
콘솔이 이미 읽은 holdings에서만 오며 `/position-management` 자신은 아무것도 부르지
않는다. 이름을 모를 때 조용히 비우는 경로도 함께 관측된 셈이다.

task 6.7은 세 항목 중 둘이 확인됐고 나머지 하나가 사용자 판정에 걸려 있어 열어 둔다.

## 배포 후 실측 완결 (2026-09-09T13:56Z, task 6.7 나머지 하나)

8월에 "측정 불가"로 남긴 다섯 값을 오늘 확인했다. 조건이 갈렸기 때문이다. 그때는
열린 포지션이 전부 격리 상태였고, 지금은 활성 격리가 전부 CLOSED 포지션에 있으며
열린 포지션은 하나도 격리되지 않았다.

원장 실측(읽기 전용, `~/.config/tossctl/journal.db`):

| 집합 | 술어 | 결과 |
|---|---|---|
| 활성 격리 | `q.released_at IS NULL AND q.position_generation = p.instance_seq` | 5건 — 466100·475150·NNE·PLTR·032820, 전부 `state=CLOSED`·`quantity=0`·`ambiguous_recovery` |
| 열린 포지션 | `p.state <> 'CLOSED'` | 3건 — IONQ 5주, RGTI 10주, TSLA 0.000154주 |

두 집합은 서로소다. 그래서 blanking 분기가 아니라 값 분기가 그려진다.

`/positions` GET (운영 컨테이너 `tossos-tossos-1`, 이미지 `5c77491d93d4`, 엔진 실행 중):

| 종목 | 익절 | 손절 | 추적 회수 | 기준 | 고점 |
|---|---|---|---|---|---|
| IONQ · 아이온큐 | 40.3300042 | 38.428393 | 39.6169 | 38.428393 | 39.73 |
| RGTI · 리게티 컴퓨팅 | 15.89098 | 15.1417 | 15.61 | 15.1417 | 15.805 |

다섯 값이 전부 숫자로 나온다. `/position-management`의 한글 종목명도 다시 보였다 —
`us · IONQ · 아이온큐`, `us · RGTI · 리게티 컴퓨팅`, `us · TSLA · 테슬라`.

GET만 보냈다. POST는 하나도 보내지 않았다.

task 6.7의 세 항목이 두 세션에 걸쳐 전부 측정됐다 — 466100 판정 격리와 한글 종목명은
2026-08-04, 다섯 값은 오늘. **6.7을 닫는다.**
