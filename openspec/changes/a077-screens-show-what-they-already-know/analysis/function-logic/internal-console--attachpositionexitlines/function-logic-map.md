...
- Base commit: `openspec/changes/a077-screens-show-what-they-already-know/base-commit.txt`
...
| `row.Quarantined()` (a077) | active quarantine on the current position generation only | `journal.ReadOnly.LivePositionExits` | read failure -> false, i.e. the pre-a077 behaviour; cannot invent a quarantine |
...
| `live protectionLiveness` (a077) | `{Wired, Running}` from the engine marker | `Console.protectionLiveness` | unwired -> the pre-a077 observation-age bound |
...
| **B7 (a077)** | `row.Quarantined()` | forces `Stale`/`StaleReason = snapshot_quarantined` | falls through | a077 2.4, 2.5 |
| **B8 (a077)** | quarantined **and** no canonical snapshot | also sets `UnknownReason` so the quarantine survives the unknown path | falls through | a077 2.4 |
| B9 | `snapshot.Snapshot != nil` | copies `Line`, `ObservationSource`, `ObservedAt` into `Source` | falls through | a077 2.1, existing fixtures |
...
The freshness call itself (line 192, `exitFreshness`) is a077's replacement for
...
| `exitFreshness` (a077) | replaces the age bound with a liveness question | pure; falls back to `WithFreshness` when the marker is unwired | `internal/console/protection_liveness.go` |
...
this function or anything it calls. a077 does not change that.

## State mutations and fallbacks

- Writes only to `rows[i]`: `ExitLine`, `ExitReference`, `StoredExit`. No package
  state, no shared map, no clock read of its own.
- Fallback direction is always toward the em dash. Both conditions a077 adds
