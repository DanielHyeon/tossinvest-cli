...
identity. It maps a snapshot or it hides it. That is unchanged by a077.
...
| B2 | `StaleReason` non-empty | status `stale`, **`StatusText = staleStatusText(reason)`** (a077), `Reason = reasonText(reason)` | **early return**, all values em dash | a077 2.3, 2.4; existing stale test |
| B3 | one share, state-only, projected zero | the one-share wording | falls through to return | existing one-share test |

**a077 changes B2 only**, and only its `StatusText`. Before: the literal
`오래된 평가` for every stale reason. After: the same literal by default, with
`판정 격리` and `엔진 정지` for the two reasons a077 introduces. The status code
...
| `staleStatusText` (a077) | the short verdict for a hidden line | pure, total; unknown reasons fall to the previous literal | `internal/operatorview/exit_line_status.go` |
...
a clock and a077 keeps that true: the freshness decision is the caller's.
