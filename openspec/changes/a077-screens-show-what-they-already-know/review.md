...
수용 근거: a077 이전에는 이 구멍이 **더 나쁜 방식으로** 가려져 있었다 — 시세가
...
a077 변경이 지워졌다.** 즉시 복구하고 변이 검증을 백업 파일 방식으로 다시 수행했으며,
...
`internal-console--console.handlepositionmanagement` 타깃은 제거했다. a079가 같은
함수를 나중에 고쳐 분기가 16개에서 20개로 늘었고, 20개 전부를 현재 트리에 묶어
RED/GREEN까지 관측한 증거가
`openspec/changes/a079-operator-can-lift-a-quarantine/analysis/function-logic/internal-console--console.handlepositionmanagement/`에
