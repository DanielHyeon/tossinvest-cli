...
| `exit_snapshot_quarantines` (a061) | unreleased rows joined on the position's current `instance_seq` | `accountActiveQuarantines` | query error -> returned; the caller shows the broker half only, so the failure is fail-closed |
...
| **B2 (a061)** | `accountActiveQuarantines` failed | none | **returns the error** | fail-closed by construction; see below |
...
| **B6 (a061)** | an active quarantine exists for this position | attaches a copy to the row | falls through | `TestAQuarantinedPositionIsNotDrawnAsProtected` |
...
| `accountActiveQuarantines` (a061) | which positions the engine has taken out of judgement | error propagates; no partial answer | AST |
...
broker call. a061 adds the third; it does not change the first two.
