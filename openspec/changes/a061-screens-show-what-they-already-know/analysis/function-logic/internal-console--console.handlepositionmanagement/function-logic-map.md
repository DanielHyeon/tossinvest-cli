...
| `c.holdings` (a061) | the holdings cache this console has already filled | `/positions` | empty -> no names, which is the pre-a061 rendering |

**Invariant**: this screen is read-only over the broker. It made no broker call
before a061 and makes none after it -- `holdingNames` peeks the cache and never
refreshes it.
...
**a061 changes two lines**: one `names := c.holdingNames(c.now())` before B10, and
...
| `Console.holdingNames` (a061) | stock names already read | no error; peeks the cache, makes no broker call | `internal/console/protection_liveness.go` |
