...
| PB4 | every package that imports an approved boundary is tainted even after the boundary erases the value to bool/error; any authority reach is rejected unconditionally | `TestApprovedCandidateBoundaryRejectsReversePrimitiveLaundering` | security re-review H1 RED fixture | focused GREEN |
...
| PB6 | a reason-only bridge cannot mask a newly reachable authority root; a046 has no exemption mechanism | `TestApprovedCandidateAuthorityReachCannotBeAllowedByBoundaryReason` | audit returned empty findings | focused GREEN |
| PB7 | direct reader remains a candidate-only stateless value evaluator and cannot accept/hold/call interface or function capability | `TestApprovedCandidatePureBoundaryRejectsInjectedAuthority`, `TestApprovedCandidatePureBoundaryAllowsValueOnlyEvaluator` | injected-authority fixture returned empty findings | focused GREEN |
| PB8 | aliases, named underlying types, embedded/fixed-array fields and generic arguments are resolved with `go/types`; package state and all non-accessor execution/mutation forms fail | `approved_boundary_types_guard_test.go` matrix | 4th review counterexamples passed the spelling guard | focused GREEN |
