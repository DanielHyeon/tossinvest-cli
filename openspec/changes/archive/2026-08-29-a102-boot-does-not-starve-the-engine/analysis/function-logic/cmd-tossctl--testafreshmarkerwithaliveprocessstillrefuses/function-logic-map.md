...
- Source: `cmd/tossctl/engineproc_test.go` (200-225)
- AST evidence: `ast.json` — AST 기준 branches **5** / returns 0 / calls 18
...
|---|---|---|---|
| B1 | `:203` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B2 | `:212` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B3 | `:215` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B4 | `:218` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B5 | `:222` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `ast.json`의 호출 18건 | 테스트 픽스처 조립과 단언 | 이 change로 바뀌지 않음 | ast.json |
