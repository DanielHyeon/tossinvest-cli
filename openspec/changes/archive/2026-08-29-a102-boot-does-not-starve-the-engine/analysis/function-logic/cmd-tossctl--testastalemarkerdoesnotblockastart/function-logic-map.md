# Function Logic Map: `TestAStaleMarkerDoesNotBlockAStart`
...
- Source: `cmd/tossctl/engineproc_test.go` (322-340)
- AST evidence: `ast.json` — AST 기준 branches **3** / returns 0 / calls 14
...
|---|---|---|---|
| B1 | `:326` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B2 | `:334` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B3 | `:337` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
