# Function Logic Map: `TestAStaleMarkerIsNotARunningEngine`
...
- Source: `internal/enginelock/enginelock_test.go` (134-148)
...
|---|---|---|---|
| B1 | `:137` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B2 | `:142` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B3 | `:145` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
