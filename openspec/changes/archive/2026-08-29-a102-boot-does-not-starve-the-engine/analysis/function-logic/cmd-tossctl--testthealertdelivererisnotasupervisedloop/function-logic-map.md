# Function Logic Map: `TestTheAlertDelivererIsNotASupervisedLoop`
...
- Source: `cmd/tossctl/a098_the_engine_registers_a_sender_test.go` (35-49)
- AST evidence: `ast.json` — AST 기준 branches **4** / returns 0 / calls 10
- Risk scan: `risk-pattern-report.md`
- source SHA-256: `1c98ccabbefe6d5b492b2d635155c048d9d15a27c983cd2b8b9a1dcea497ebaa`
...
|---|---|---|---|
| B1 | `:37` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B2 | `:41` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B3 | `:44` range | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B4 | `:45` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `ast.json`의 호출 10건 | 테스트 픽스처 조립과 단언 | 이 change로 바뀌지 않음 | ast.json |
