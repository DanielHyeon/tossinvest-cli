# Function Logic Map: `TestAHeldMarkerReadsAsRunning`
...
- Source: `internal/enginelock/enginelock_test.go` (107-129)
- AST evidence: `ast.json` — AST 기준 branches **5** / returns 0 / calls 15
- Risk scan: `risk-pattern-report.md`
- source SHA-256: `d460165ffc609568b4ff940dd12227409e2a20e802f9297ebdc1fa5b01945c1d`
- **a102가 바꾼 것**: `enginelock.Hold`의 D4 반환(핸들)에 맞춰 수신 변수와 `defer`를 바꿨다. 단언은 그대로다.
...
|---|---|---|---|
| B1 | `:111` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B2 | `:117` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B3 | `:120` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B4 | `:123` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |
| B5 | `:126` if | 테스트 코드 — 부수효과 없음 | 이 change로 바뀌지 않음 |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `ast.json`의 호출 15건 | 테스트 픽스처 조립과 단언 | 이 change로 바뀌지 않음 | ast.json |
