...
- 이 change의 base: `137cc8d0` — 본문 변경: `console.Options` 리터럴에 `Orders`·`Signals` 두 seam 배선 추가 (`GateLimits`는 base에 이미 있었다) + 읽기 seam이 공유하는 계좌 해석기(`newConsoleBroker`) 1개 (revision=current)
