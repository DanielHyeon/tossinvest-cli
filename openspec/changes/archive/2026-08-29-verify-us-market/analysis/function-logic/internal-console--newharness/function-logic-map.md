...
|---|---|---|---|---|
| B1 | `for _, f := range tweak {` (internal/console/console_test.go:111, range) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
| B2 | `if err != nil {` (internal/console/console_test.go:116, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
| B3 | `if err != nil {` (internal/console/console_test.go:121, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
