# Function Logic Map: `Console.handleOverview`
...
The page struct now embeds the shared `chrome`, so this handler builds it and the template reads the four status facts from it. What the handler reads, what it gates on and what it renders are unchanged. Also reads the holdings cache a second time — peek, so no broker call — to give the strip an instant, and promotes a running verification to a named refresh hold so the cell shows the reason instead of a warning tone.

| Branch | Condition | Mutation/side effect | Return/error | Required test |
|---|---|---|---|---|
| B1 | if at line 1174 | no domain mutation; a GET render | unchanged behaviour | `TestEveryScreenRendersTheSameStatusStrip`, `TestEachScreenKeepsItsOwnReloadPeriod`, `TestEveryScreenIsCalledOneThing` |
