# Function Logic Map: `Console.handleSignals`
...
The page struct now embeds the shared `chrome`, so this handler builds it and the template reads the four status facts from it. What the handler reads, what it gates on and what it renders are unchanged. Passes the discovery reading's freshness, which carries no TTL: this value advances on a tick the console neither causes nor observes, so it is given a reason rather than a tone.
