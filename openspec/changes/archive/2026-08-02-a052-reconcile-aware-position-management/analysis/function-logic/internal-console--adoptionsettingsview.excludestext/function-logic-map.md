# Function Logic Map: `adoptionSettingsView.ExcludesText`
...
|---|---|---|---|
| Function contract | `adoptionSettingsView.ExcludesText(params=0, results=1)` | current Go signature and callers | errors/unknown values propagate without inventing effective state |
...
|---|---|---|---|---|---|
| B1 | `if` at line 103 | `if !v.Known {` | none beyond calls | early return/error nearby | go test ./internal/console |
| B2 | `if` at line 106 | `if len(v.ExcludeSymbols) == 0 {` | none beyond calls | early return/error nearby | go test ./internal/console |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `len` | explicit dependency at line 106 | result/error follows the AST-recorded branch; no hidden retry is assumed | current AST + focused tests |
| `strings.Join` | explicit dependency at line 109 | result/error follows the AST-recorded branch; no hidden retry is assumed | current AST + focused tests |
