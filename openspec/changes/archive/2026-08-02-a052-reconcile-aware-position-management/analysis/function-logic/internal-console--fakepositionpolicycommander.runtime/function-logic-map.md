# Function Logic Map: `fakePositionPolicyCommander.Runtime`
...
|---|---|---|---|
| Function contract | `fakePositionPolicyCommander.Runtime(params=1, results=2)` | current Go signature and callers | errors/unknown values propagate without inventing effective state |
...
|---|---|---|---|---|---|
| B1 | branchless happy path | execute `fakePositionPolicyCommander.Runtime(params=1, results=2)` | deterministic value/call effects only | normal function result | fakePositionPolicyCommander.Runtime |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `f.mu.Lock` | execute the explicit dependency at line 30 | callee result/error is handled by the AST-recorded branch; no hidden retry is assumed | current AST + focused package tests |
| `f.mu.Unlock` | execute the explicit dependency at line 31 | callee result/error is handled by the AST-recorded branch; no hidden retry is assumed | current AST + focused package tests |
