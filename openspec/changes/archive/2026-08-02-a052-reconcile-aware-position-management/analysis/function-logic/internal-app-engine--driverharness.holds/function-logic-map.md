# Function Logic Map: `driverHarness.holds`
...
|---|---|---|---|
| Function contract | `driverHarness.holds(params=4, results=0)` | current Go signature and callers | errors/unknown values propagate without inventing effective state |
...
|---|---|---|---|---|---|
| B1 | branchless happy path | execute `driverHarness.holds(params=4, results=0)` | deterministic value/call effects only | normal function result | driverHarness.holds |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `h.holdsMarket` | execute the explicit dependency at line 177 | callee result/error is handled by the AST-recorded branch; no hidden retry is assumed | current AST + focused package tests |

## State mutations and fallbacks

- AST records 0 assignment(s), 0 return statement(s), and 0 goroutine launch(es).
