# Function Logic Map: `contractReader.Optimization`
...
|---|---|---|---|
| Function contract | `contractReader.Optimization(params=1, results=2)` | current Go signature and callers | errors/unknown values propagate without inventing effective state |
...
|---|---|---|---|---|---|
| B1 | `if` at line 52 | `if err != nil {` | local/read-model state only; see AST assignments | early return/error path nearby | contractReader.Optimization |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `optimization.CoreRegistry` | execute the explicit dependency at line 51 | callee result/error is handled by the AST-recorded branch; no hidden retry is assumed | current AST + focused package tests |

## State mutations and fallbacks

- AST records 1 assignment(s), 2 return statement(s), and 0 goroutine launch(es).
