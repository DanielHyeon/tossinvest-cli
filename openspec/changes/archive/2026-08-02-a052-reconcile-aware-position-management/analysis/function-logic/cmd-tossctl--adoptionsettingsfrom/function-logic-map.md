# Function Logic Map: `adoptionSettingsFrom`
...
|---|---|---|---|
| Function contract | `adoptionSettingsFrom(params=1, results=1)` | current Go signature and callers | errors/unknown values propagate without inventing effective state |
...
|---|---|---|---|---|---|
| B1 | branchless happy path | execute `adoptionSettingsFrom(params=1, results=1)` | deterministic effects only | normal result | go test ./cmd/tossctl |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `append` | explicit dependency at line 419 | result/error follows the AST-recorded branch; no hidden retry is assumed | current AST + focused tests |
| `call` | explicit dependency at line 419 | result/error follows the AST-recorded branch; no hidden retry is assumed | current AST + focused tests |
