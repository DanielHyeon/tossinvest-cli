...
## 개정 2 (2026-08-05) — 독립 리뷰

### D5 — 이름은 안정화 전에 배운다 (issues.md B1)

개정 1은 **안정된** 스냅샷에서만 이름을 배웠다. 안정화는 `StabilisationInterval`
이상 떨어진 두 수집이 일치해야 하고, 움직이는 계좌에서는 몇 분이 걸린다. 그런데
exit 루프는 같은 순간에 시작해 5초마다 돌고, 그 알림 latch는 **포지션당·프로세스당
1회**이며 다시 무장되지 않는다.

결과: 한 포지션이 평생 받는 그 한 번의 알림이 이름을 알기 전에 나갈 확률이 높다.
코드만 찍힌다 — a085가 고치려던 그 결함이, 들어갈 필요도 없던 경주에서 진 것이다.

**규칙: 수집할 때마다 배운다.** `stabilise`의 두 `Collect` 각각 뒤에서 `learnNames`를
부른다. 요청 수는 그대로다(같은 응답을 읽을 뿐이다). 부수 효과로 **끝내 안정화되지
않는 계좌도 이름을 안다** — 그 계좌가 알림이 가장 많이 나가는 계좌다.

메모리 전용이라는 성질은 유지한다. 재시작마다 다시 배우고, 첫 수집까지의 창은 2초가
아니라 한 번의 holdings 응답으로 줄어든다.

### D6 — a085의 §0.8 주장은 검사하는 것으로 좁힌다 (issues.md B2)

`TestTheUnmanagedAlertNamesTheStockInKorean`은 "§0.8: 계좌는 운영자 휴대폰에 도달하는
알림에 타지 않는다"고 적고 `Event.Title+Body`만 봤다. **발행되는 표면이 아니다.**
`obs.Notifier`가 구조화 `Fields`를 발행 본문에 덧붙이고 `Fields[account]`는 브로커
실계좌번호다.

이 노출은 a085 이전부터 있었고 `internal/obs`에 있으며 이 diff가 건드리지 않는다.
따라서 a085에서 하는 일은 **주장을 검사와 일치시키는 것**뿐이다 — 테스트는 "a085가
쓴 산문이 계좌를 흘리지 않는다"만 단언하고, 노출 자체는 `issues.md` B2에 별도 change
후보로 남긴다. 검사하지 않는 보장을 이름 붙인 테스트는 이름을 덜 붙인 테스트보다 나쁘다.
