...
5개 지점 전건 닫힘 확인 — risk-management MODIFIED는 축자 대조(수정 = 한 문단+시나리오 1건, 나머지 바이트 동일)로 충실성 검증. 착수 전 편집 1건 반영:

- **P2 flock 순서(TOCTOU)**: 채택 — flock 획득을 기동 첫 동작으로 이동(journal open·마이그레이션 전체가 배타 안으로 — 바이너리 업그레이드 직후 autostart+버튼 동시 기동이 정확히 이 경로였다)
- **P3 비-Unix**: 채택 — 정직한 거부 선례 명시(task 1.1)

3라운드 누적 P1 5·P2 6 전건 종결. **freeze 확정 — 구현 착수 승인.**
