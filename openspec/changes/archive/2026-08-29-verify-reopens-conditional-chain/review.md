...
2026-07-26에 정확히 그 이유로 체인 전체가 skipped였다. 코드로 막을 수 있는 것이 아니다.

**2026-07-29 해소**: 사용자가 KR 보유(일승 333430)를 확인했고 콘솔 `/positions`에 있다.
기록도 독립적으로 뒷받침한다 — 2026-07-28에 등록된 조건주문 `grLKqiGuCVS7mj`의 symbol이
`333430`이고, 그것은 `firstUsableHolding`이 그 보유를 골랐다는 뜻이다. 전제 충족.
