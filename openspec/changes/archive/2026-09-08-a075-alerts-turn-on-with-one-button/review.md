...
## 완료 게이트 5단계 면제 (2026-09-08)

`배포 후 실측` 태스크를 닫으면서 완료 게이트를 다시 돌렸고, **5단계(Function Logic
Map)만 실패했다.** 이것을 사유를 적고 면제한다 — 침묵한 생략이 아니다.

| 게이트 단계 | 오늘 결과 |
|---|---|
| 1~4 (tasks 존재 · 미완료 0 · 짝 선언 · review 존재) | 통과 |
| 5 (Function Logic Map) | **실패 — 아래 사유로 면제** |
| 6~11 (sdd-check · test · test-seams · test-race · vet · validate) | 따로 돌려 6/6 exit 0 |

**실패의 내용.** `check_analysis.py` 의 `changed_existing_functions` 는
`git diff <base> -- '*.go'` 를 **워킹트리**에 대고 돈다. 이 change 의 base 는
2026-08-04 재기준화가 박은 `448dfeb1`(위 §병합 후 재기준화)이고 오늘 HEAD 는
`62b35779` 다. 그 사이에 a112 와 `origin/main` 병합이 들어왔으므로, 그것들이 고친
함수가 전부 "이 change 가 증거를 내지 않은 함수"로 집계된다. 지적 목록에
...
1. 이 세션은 **Go 파일을 0개** 바꿨다(`git diff --name-only` 에 `.go` 없음). 닫은
   태스크는 사람이 콘솔에서 잰 것을 적은 마크다운뿐이다.
2. 5단계는 이 change 가 자기 base 위에 서 있던 시점에 **이미 통과했고** 그 기록이
   완료 태스크로 남아 있다(§9/§7 의 `make gate` 줄).
3. 같은 실패가 **손대지 않은** a074(316) · a077(318) · a079(317) 에도 그대로 있다.
   즉 이것은 이 change 의 상태가 아니라 저장소의 상태다.

**재기준화를 다시 하지 않은 이유.** 위 §병합 후 재기준화가 선례이고 절차도 있다.
그러나 그 절차는 `revision: current` AST 번들을 병합된 내용으로 재추출하고 분기 ID 를
다시 매기는 일을 포함한다. 8월 이후 이 change 의 Go 코드는 움직이지 않았으므로 그
재도출이 새로 증명하는 것은 없고, 증명하려는 것은 이미 (2)에 기록돼 있다. 반면
재번호는 위치 기반이라 조용히 틀릴 수 있다.

**옳은 해결은 도구 쪽이다.** 5단계가 base→**워킹트리**가 아니라 base→**그 change 가
착지한 커밋**을 보면 이 물음은 병합 뒤에도 뜻을 유지한다. 지금은 change 가 트리의
끝일 때만 답이 성립한다. 다섯 change 가 여기 걸려 있고 앞으로 모든 `배포 후 실측`
태스크가 같은 모양이므로 후속 change 로 연다.
