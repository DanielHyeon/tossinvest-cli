...
- 안전 불변식 §0 위반 여부 검토: **조건부 통과.** 통과 조건 넷:
    ...
    (4) **`RestartSoak`의 반환 계약을 바꾸지 않는다.** 승인 기록 실패를 에러로 승격시키면
        ...
        방금 선 서베이를 죽인다.
    (5) **부팅은 버튼이 아니다 — 리뷰가 추가한 조건(2026-08-12).** 부팅 경로는
        (a) 이미 돌고 있는 서베이를 죽이지 않고, (b) `PrepareSpawn`을 넘기지 않는다.
        근거는 `startEngine`이 이미 같은 선을 긋는다는 것(떠 있으면 거절)과,
        `spawnDetachedSoak`의 `setsid`가 서베이를 콘솔보다 오래 살게 하려고 존재한다는
        것이다. 토큰 캐시는 a082가 만든 조율 지점이라 이유 없이 지우면 안 된다.
```
...
(5)는 **절반만 측정된다.** (a)는 `bootSurvey`가 별도 함수이므로 100.0%로 측정된다
(`TestBootSurveyLeavesARunningSurveyAlone`). (b)는 두 closure의 차이가 `runConsole` 안에
있으므로 측정되지 않는다 — `bootSurvey`는 `start` seam을 인자로 받아서 **언제 부르는지는
알지만 무엇을 넘겼는지는 모른다.** (b)는 코드 리뷰 조건으로 남는다.

### 이 선언이 처음에 놓친 것

조건 (5)는 **처음 선언에 없었다.** 없었기 때문에 첫 구현이 부팅을 버튼과 같은 seam으로
보냈고, 그것이 리뷰가 잡은 결함이다. 선언이 "무엇을 건드리면 안 되는가"만 적고 "이 경로가
버튼과 같은 행위인가"를 묻지 않은 것이 원인이다.
