...
## ✅ 편집 후 재측정 — 예측이 그대로 맞았다 (2026-08-12, task 4.3.3)

`ReasonAlertSenderDown` 한 줄을 리터럴에 더한 뒤 AST 를 다시 뽑았다.

| | 편집 전 | **편집 후** | 예측 |
|---|---|---|---|
| 분기 | 0 | **0** | 맞음 |
| 이탈 | 2 | **2** | 맞음 |
| 호출 | 1 (`sort.Slice`) | **1** | 맞음 |
| 범위 | `:254-301` | **`:254-302`** | 한 줄 늘었다 |
| source SHA-256 | `cafda865…` | **`5c1a1f86…`** | — |

**분기가 안 늘어난 것이 이 편집이 리터럴 편집이라는 증거다.** 늘었다면 누군가
등록에 조건을 걸었다는 뜻이고, 그것은 *"이 빌드가 낼 수 있는 모든 사유 코드"*라는
이 함수의 doc 을 빌드 시점 조건부로 만든다.

**열거 길이는 29 → 30 이다** (`wc -l testdata/reason_codes.golden` 재측정).
상수 선언은 45 → **46** 이고, 즉 **미등록 상수는 여전히 16 개**다 —
그것은 a098 의 범위가 아니고 tasks 의 「안 하는 것」 표가 이름을 붙여 둔다.
