# Function Logic Map: `engineFakes.install`
...
프로세스 seam 대체. a059에서 lookup이 어떤 journal 디렉터리를 질문받았는지 기록하도록 넓혔다.

| Branch | Condition | Mutation/side effect | Return/error | Required test |
|---|---|---|---|---|
| B1 | if at line 55 | 없음 — 테스트 코드 | unchanged behaviour | 이 함수를 통과하는 `cmd/tossctl` 테스트 |
| B2 | if at line 59 | 없음 — 테스트 코드 | unchanged behaviour | 이 함수를 통과하는 `cmd/tossctl` 테스트 |
| B3 | if at line 67 | 없음 — 테스트 코드 | unchanged behaviour | 이 함수를 통과하는 `cmd/tossctl` 테스트 |
| B4 | if at line 71 | 없음 — 테스트 코드 | unchanged behaviour | 이 함수를 통과하는 `cmd/tossctl` 테스트 |
