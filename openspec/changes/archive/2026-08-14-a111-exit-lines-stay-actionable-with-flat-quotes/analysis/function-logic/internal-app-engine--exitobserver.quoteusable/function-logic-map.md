# Function Logic Map: `ExitObserver.quoteUsable`
...
|---|---|---|---|---|
| B1 | AST `if` at `internal/app/engine/exitloop.go:1073`; reject wall evidence from the future and require clock.LeaseElapsed to remain at the inclusive price-evidence bound | limited to the stated seam; no order authority added | typed/read-only fail-closed result | `TestA111QuoteUseLeaseCannotBeExtendedByWallClockRollback` |
| Return | all admitted paths | reject wall evidence from the future and require clock.LeaseElapsed to remain at the inclusive price-evidence bound | exact function result | `TestA111QuoteUseLeaseCannotBeExtendedByWallClockRollback` |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| direct A111 time/evidence collaborators | reject wall evidence from the future and require clock.LeaseElapsed to remain at the inclusive price-evidence bound | no clock movement may extend a lease or upgrade a stopped marker | AST + named RED |

## State mutations and fallbacks

- reject wall evidence from the future and require clock.LeaseElapsed to remain at the inclusive price-evidence bound.
