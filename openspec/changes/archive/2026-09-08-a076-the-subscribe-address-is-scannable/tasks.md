...
- [x] 8.1 배포 후 실제 폰 카메라로 카드의 QR을 찍어 ntfy 앱이 열리는지 실측한다.

      **확인 (2026-09-08, 이미지 `5c77491d93d4`).** `/settings/tools`가 그린
      `viewBox="0 0 41 41"`(버전 6) 심볼을 240×240으로 렌더한 것을 사용자가 실제 폰
      카메라로 찍었고 열렸다. 기록은 `issues.md` §배포 후 실측이며, 같은 절이 이것으로
      I3가 닫히고 거기 적힌 마스크 벌점 의심이 실현되지 않았음을 적는다.
