...
- Inputs are `TestLiveOrdersForSymbolUsesLegacyTerminalOnlyWithinItsMarket(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills_test.go:1036 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/journal/fills_test.go:1039 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/journal/fills_test.go:1042 | Preserve the condition, error propagation, and fail-closed behavior. |
| B4 | if | internal/journal/fills_test.go:1045 | Preserve the condition, error propagation, and fail-closed behavior. |
| B5 | if | internal/journal/fills_test.go:1054 | Preserve the condition, error propagation, and fail-closed behavior. |
| B6 | if | internal/journal/fills_test.go:1060 | Preserve the condition, error propagation, and fail-closed behavior. |
| B7 | if | internal/journal/fills_test.go:1063 | Preserve the condition, error propagation, and fail-closed behavior. |
