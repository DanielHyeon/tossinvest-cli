...
- Inputs are `Journal.NetPositions(params=1, results=2)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills.go:1480 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | for | internal/journal/fills.go:1486 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/journal/fills.go:1488 | Preserve the condition, error propagation, and fail-closed behavior. |
| B4 | if | internal/journal/fills.go:1492 | Preserve the condition, error propagation, and fail-closed behavior. |
| B5 | if | internal/journal/fills.go:1495 | Preserve the condition, error propagation, and fail-closed behavior. |
| B6 | if | internal/journal/fills.go:1500 | Preserve the condition, error propagation, and fail-closed behavior. |
| B7 | range | internal/journal/fills.go:1505 | Preserve the condition, error propagation, and fail-closed behavior. |
...
- AST assignment points: 9; return points: 4; deferred operations: 1.
