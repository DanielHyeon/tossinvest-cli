...
- Inputs are `TestOrphanSweepDoesNotReleaseReservationBoundToAnotherDecision(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/reservation_sweep_test.go:414 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/journal/reservation_sweep_test.go:422 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/journal/reservation_sweep_test.go:430 | Preserve the condition, error propagation, and fail-closed behavior. |
| B4 | if | internal/journal/reservation_sweep_test.go:433 | Preserve the condition, error propagation, and fail-closed behavior. |
| B5 | if | internal/journal/reservation_sweep_test.go:436 | Preserve the condition, error propagation, and fail-closed behavior. |
