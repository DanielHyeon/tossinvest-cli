...
- Inputs are `TestNewMismatchLatchesTheGateBeforeJournalIO(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:674 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/reconcile/restore_test.go:677 | Preserve the condition, error propagation, and fail-closed behavior. |
