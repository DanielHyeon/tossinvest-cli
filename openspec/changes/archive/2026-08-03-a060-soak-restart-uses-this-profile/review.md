...
## 운영자 승인 후속 처분 — 외부 soak autostart 은퇴

구현과 컨테이너 실측이 끝난 뒤 운영자가 만료된 1회성
`~/.local/share/tossos/bin/soak-autostart.sh` 제거를 별도로 지시했다. 이는 proposal의
초기 구현 Non-Goal을 운영자 승인으로 확장한 후속 운영 처분이다.

제거 전 다음을 확인했다.

- 실행 중인 soak 프로세스가 없었다.
- systemd unit, cron, shell rc, `console-launch.sh`에 실행·등록 참조가 없었다.
- 파일은 최대 24시간짜리 1회성 감시자였고 2026-07-30 이후 만료 상태였다.

저장소에는 그 설치물의 역사와 설계 근거를 설명하는 **텍스트 참조**가 남는다. 그러므로
검증 결과를 "저장소 전체에 참조가 없다"가 아니라 "활성 실행·등록 참조가 없다"로
한정한다. soak의 현재 drift 계약은 외부 파일이 아니라 `soakArgs`가 만드는 argv와
`soakProcessPattern` 사이에 있다. 런타임 코드·패턴·운영 토글은 바꾸지 않았다.

처분 확인 중 발견한 attestation timer/profile 불일치는 a060 범위에 섞지 않고
`STORY-TOS-a063` / `a063-align-attestation-renewal-profile`로 분리했다. 이 후속은
2026-08-29 만료 전에 구현·검증해야 하며, 이번 처분에서는 timer나 운영 설정을 바꾸지
않았다.

## 통합 기준 재고정

a060 런타임 구현과 컨테이너 실측 커밋은 이미 `main`에 포함돼 있었고, 이번 잔여 통합은
외부 설치물 처분 기록과 코드 주석만 추가한다. 기존 `base-commit.txt`를 유지하면 그 뒤
`main`에 들어온 a061/a062의 무관한 Go 함수 수백 건을 a060 분석 누락으로 계산한다.
따라서 a056·a055 통합 때 사용한 저장소 선례와 같은 방식으로 비교 기준을 최종 통합 직전
`main` `4e7c800d0722b03411928c03ff9fbdf89536d0c4`로 재고정했다. 이 기준 이후 실행문 변경은
없으며, `cmd/tossctl/soakproc*.go` 변경은 주석뿐이다.
