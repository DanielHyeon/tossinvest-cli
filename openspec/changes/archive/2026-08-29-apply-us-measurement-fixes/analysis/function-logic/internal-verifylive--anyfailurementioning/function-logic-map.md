# Function Logic Map: `anyFailureMentioning`
...
|---|---|---|---|---|
| B1 | `for _, e := range entries {` (internal/verifylive/fake_broker_test.go:870, range) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
| B2 | `if e.Verdict == VerdictFail && strings.Contains(e.Reason, text) {` (internal/verifylive/fake_broker_test.go:871, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
