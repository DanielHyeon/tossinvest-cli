...
- [x] 3.4 `token()`이 캐시 파일 mtime을 보고, 바뀌었으면 메모리 캐시를 믿지 않고
      디스크를 다시 읽는다 (design D2).
- [x] 3.5 3.4가 없으면 회전마다 401을 한 번 먹는 것을 테스트로 고정한다.
...
- [ ] 7.5 별도 컨텍스트의 독립 리뷰 — 인증 경로는 High-risk이므로 적대적 Eng
