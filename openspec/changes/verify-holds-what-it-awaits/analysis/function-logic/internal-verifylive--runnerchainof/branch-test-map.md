# Branch Test Map: `Runner.chainOf`
...
|---|---|---|---|---|
| B1 | `range` line 758: `for _, entries := range [][]Entry{{{Artifacts: sr.artifacts}}, r.written, r.prior} {` | `Runner.chainOf` 계열 테스트 | 아래 RED 기록 | yes |
| B2 | `if` line 759: `if c := ChainOf(entries, kind, id); c != "" {` | `Runner.chainOf` 계열 테스트 | 아래 RED 기록 | yes |
