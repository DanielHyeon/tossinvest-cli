# Branch Test Map: `stepRun.markHeld`
...
|---|---|---|---|---|
| B1 | `range` line 740: `for i := range sr.artifacts {` | `stepRun.markHeld` 계열 테스트 | 아래 RED 기록 | yes |
| B2 | `if` line 741: `if sr.artifacts[i].Kind == kind && sr.artifacts[i].ID == id && !sr.artifacts[i].Cancelled {` | `stepRun.markHeld` 계열 테스트 | 아래 RED 기록 | yes |
