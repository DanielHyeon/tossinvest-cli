...
|---|---|---|---|---|
| B1 | base `if` at `internal/exitpolicy/decimal.go:63` | base behavior is preserved or deliberately replaced by the adjacent current function | existing base return/error contract | base-revision compatibility and affected-package regression tests |
| B2 | base `if` at `internal/exitpolicy/decimal.go:66` | base behavior is preserved or deliberately replaced by the adjacent current function | existing base return/error contract | base-revision compatibility and affected-package regression tests |
| B3 | base `if` at `internal/exitpolicy/decimal.go:70` | base behavior is preserved or deliberately replaced by the adjacent current function | existing base return/error contract | base-revision compatibility and affected-package regression tests |
