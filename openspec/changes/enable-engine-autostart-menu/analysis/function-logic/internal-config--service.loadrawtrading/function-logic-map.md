# Function Logic Map: `Service.LoadRawTrading`
...
|---|---|---|---|---|
| B1 | if at line 93 | Only the guarded return/call in `Service.LoadRawTrading` | branch result or propagated error | existing operating-config regression tests |
| B2 | if at line 96 | Only the guarded return/call in `Service.LoadRawTrading` | branch result or propagated error | existing operating-config regression tests |
| B3 | if at line 102 | Only the guarded return/call in `Service.LoadRawTrading` | branch result or propagated error | existing operating-config regression tests |
...
- Safe edit boundary: `Service.LoadRawTrading` and its typed seam only.
