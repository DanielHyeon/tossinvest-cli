# Function Logic Map: `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild`
...
|---|---|---|---|---|
| B1 | range at line 118 | Only the guarded return/call in `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild` | branch result or propagated error | `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild` (self-verifying test) |
| B2 | if at line 123 | Only the guarded return/call in `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild` | branch result or propagated error | `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild` (self-verifying test) |
...
- Safe edit boundary: `TestEngineArgsCarryTheExplicitSessionFileIntoTheChild` and its typed seam only.
