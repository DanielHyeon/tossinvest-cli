...
- Source: `cmd/tossctl/engine.go` (**347-430**) — 4.2 편집 전에는 `331-414`
  
  > **⛔ 줄이 세 번 밀렸고, 세 번이 다 다른 이유였다.**
  > 4.2는 이 함수 **안**을 편집해 길이를 바꿨고(→ `331-414`),
  > 4.4b-1은 이 함수 **위**의 `runEngineRun`을 편집해 통째로 밀었고(→ `343-426`),
  > 4.4b-2는 더 위의 `newEngineCmd`에 하위 명령 하나를 등록해 또 밀었다(→ `347-430`).
  > 뒤의 두 번은 **본문이 한 자도 안 바뀌었는데 인용이 전부 틀리는** 종류다 —
  > 분기·이탈·호출은 세 시점 내내 6·8·11 그대로이고 그것이 「안 건드렸다」의 증거다.
  > 그리고 4.2 시점의 헤더는 끝줄을 `388`로 적고 있었다(실측 `414`). 재서 고쳤다.
  >
  > **⚠ 세 번이면 우연이 아니다.** 한 파일 안의 함수를 편집할 때마다 그 아래
  > 모든 번들이 stale 이 된다 — `check_analysis`가 매번 잡았고, 매번 **인용만**
  > 다시 쟀다. 값이 아니라 위치가 흔들리는 것이므로 카운트가 그 판정을 진다.
- AST evidence: `ast.json` — 편집 후 branches **6** / returns **8** / calls 11 / assignments 7
  (편집 전 5 / 7 / 10 / 6)
- Risk scan: `risk-pattern-report.md`
- source SHA-256: **`f13e36b3…`** (4.4b-1 시점 `301f68c6…` · 4.2 시점 `8c668aac…`)

## ⛔ 이 번들은 §5.2의 표에 **없던 자리**다 — 왜 생겼는지 먼저 적는다

§5.2는 `cmd/tossctl` diff를 **4.4의 운영자 표면**으로만 적고 있었다. 그런데 배달
실행자를 프로덕션에서 **실제로 돌게 하는** 한 줄은 여기 있다 —
`RuntimeOptions`를 만드는 유일한 자리가 이 함수이기 때문이다(`engine.go:394`).
...
|---|---|---|---|
| B1 | `:349` | `engineFillDetector` 오류 | `:350` |
| B2 | `:358` | `ReconcileDriver` 오류 | `:359` |
| B3 | `:369` | `ExitObserver` 오류 | `:370` |
| B4 | `:374` | `Recovery` 오류 | `:375` |
| B5 | `:378` | `NewRefreshingPairedStrategyEntrySupervisor` 오류 | `:379` |
| **B6 (a098 신설)** | `:390` | **`ectx.AlertDeliverer(clk)` 오류** | `:391` |
| (분기 아님) | `:394`·`:403` | `NewRuntime(...)` 반환 | — |
