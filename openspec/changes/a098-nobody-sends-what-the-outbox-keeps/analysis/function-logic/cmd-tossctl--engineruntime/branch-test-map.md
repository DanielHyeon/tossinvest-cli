...
|---|---|---|---|---|
| B1 | `:333` 체결 감지 구성 실패면 거절 | 기존 분기 테이블 | no | **yes (기존)** |
| B2 | `:342` 대사 드라이버 구성 실패면 거절 | 같은 테스트 | no | **yes (기존)** |
| B3 | `:353` exit 관측 구성 실패면 거절 | 같은 테스트 | no | **yes (기존)** |
| B4 | `:358` 복구 시퀀스 구성 실패면 거절 | 같은 테스트 | no | **yes (기존)** |
| B5 | `:362` `strategy-entry` 구성 실패면 거절 | 같은 테스트 | no | **yes (기존)** |
| B6 | **신설** `:374` — **배달 실행자 구성 실패면 거절**(fail-closed) | — | — | **아래 ⛔ — 덮이지 않았다** |
| 이탈 `:387` | 정상 조립 — 감독 넷 + **보조 하나** | **a098** `TestProductionRuntimeStartsExactlyOneAlertDeliverer` · `TestTheAlertDelivererIsNotASupervisedLoop` · 기존 `TestProductionRuntimeIncludesOneDormantStrategyEntryOuterLoop` | **yes — 뮤테이션 L**: 실행자를 만들고 `Auxiliary`에 안 넘기면 `auxiliary names=[]`로 FAIL | **yes (2026-08-12)** |
