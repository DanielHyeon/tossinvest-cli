# Function Logic Map: `goneSubjectRecord`

- Source: `internal/verifylive/redo_test.go`
- Function: `internal/verifylive/redo_test.go:goneSubjectRecord`
...
|---|---|---|---|---|
| B1 | `range` (line 141) — `for _, step := range Steps() {` | 없음 | 루프/분기 계속 | 아래 참조 |
| B2 | `if` (line 142) — `if v, ok := dependents[step.ID]; ok {` | 없음 | 루프/분기 계속 | 아래 참조 |

## Calls and live bindings

| Callee | Why called | Error/timeout/retry contract | Evidence |
|---|---|---|---|
| `Steps` | ast.json calls (line 141) | 순수 호출 또는 테스트 헬퍼 — error 계약은 호출부에서 처리 | ast.json |
| `append` | ast.json calls (line 143) | 순수 호출 또는 테스트 헬퍼 — error 계약은 호출부에서 처리 | ast.json |
