# Branch Test Map: `goneSubjectRecord`
...
|---|---|---|---|---|
| B1 | `range` line 141: `for _, step := range Steps() {` | `goneSubjectRecord` | no | yes |
| B2 | `if` line 142: `if v, ok := dependents[step.ID]; ok {` | `goneSubjectRecord` | no | yes |
