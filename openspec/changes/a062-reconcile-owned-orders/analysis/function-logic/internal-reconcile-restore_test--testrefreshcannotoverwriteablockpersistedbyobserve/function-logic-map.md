...
- Inputs are `TestRefreshCannotOverwriteABlockPersistedByObserve(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | select | internal/reconcile/restore_test.go:291 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/reconcile/restore_test.go:299 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/reconcile/restore_test.go:302 | Preserve the condition, error propagation, and fail-closed behavior. |
| B4 | if | internal/reconcile/restore_test.go:305 | Preserve the condition, error propagation, and fail-closed behavior. |
| B5 | if | internal/reconcile/restore_test.go:308 | Preserve the condition, error propagation, and fail-closed behavior. |
...
- `time.After`: errors and state follow mapped branches.
