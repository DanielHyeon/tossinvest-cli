...
A screen that reloads itself cannot keep a native <details> open, so its explanatory disclosures are driven by a URL parameter that the reload carries with it. The parameter is display-only and reaches no judgement, save or audit record. Reads the fold state out of the request. Nothing else changed: the same filter choice, the same orders read, the same freshness, the same Refresh.
