# engine-safety — a065 delta

> **ADDED만 쓴다.** a064의 "알림 전송 경로는 설정 가능하고 그 설정은 audit된다"는
> 그대로 유효하고 여기서 넓어지지 않는다. a065가 더하는 것은 그 설정을 **누가 만드는가**에
...
식별자의 생성과 저장은 audit 로그에 시각·주체와 함께 기록되어야 하며 (SHALL), 기록되는 것은 채널이 설정되었다는 사실이지 식별자의 값이 아니다 (SHALL NOT — §0.8, a064가 정한 계약과 같다).
