...
## 4차 — 배포와 컨테이너 실측 (2026-08-05)

사람이 직접 승인한 배포다. 커밋 `df4407ed`가 `main`에 land했고 그 코드로 만든
이미지 `tossos@sha256:b083bc72bff968b7db00127d59e32d3a05bbb7155bce756a3c85ae8d9fb8f517`
를 `httpapi` → `tossos` 순서로 하나씩 교체했다. 두 service 모두 healthy로 돌아왔고
엔진은 `reconcile`, `exit`, `filldetect`, `strategy-entry` 네 루프를 모두 올렸다.
journal schema는 교체 전후 모두 v29라 마이그레이션이 없었다.

교체 전 이미지에는 없고 교체 후에만 있는 심볼로 배포 동일성을 확인했다.

| 심볼 | 교체 전 `efba00b5` | 교체 후 `b083bc72` |
|---|---|---|
| `positionPolicyCache` | 0 | 8 |
| `refreshLifecycleLocked` | 0 | 2 |

### task 6.6 — 렌더 횟수와 엔진 도달 횟수의 분리

두 화면을 번갈아 20회씩 두 묶음 렌더했다. 각 묶음은 약 1.3초에 끝나므로 `Runtime`
간격(5초) 하나에 20회가 들어간다.

| 묶음 | 직전 렌더로부터 | 렌더 | 최소 | 중앙값 | 최대 | 100ms 초과 |
|---|---|---|---|---|---|---|
| 1 (간격 만료 상태) | 5초 이상 | 20 | 7.8ms | 22.9ms | **402.6ms** | **1** |
| 2 (묶음 1 직후) | 1초 미만 | 20 | 6.4ms | 14.7ms | 26.4ms | **0** |

묶음 1에서 엔진에 실제로 닿은 렌더는 **20회 중 1회**다. 나머지 19회와 묶음 2의
20회는 공유된 reading을 받았다. 캐시가 없으면 40회 전부가 descriptor 재해석 + dial
+ 엔진 쓰기 커넥션 위의 SELECT를 냈을 자리다.

### 엔진 사이클이 나빠지지 않았다

같은 시간대 `reconcile` 루프의 완주 간격이다. 묶음 1(00:49:51)과 묶음
2(00:50 직후)가 `00:49:00`→`00:50:02` 구간을 지난다.

```
00:44:48  00:45:50  00:46:53  00:47:57  00:49:00  00:50:02  00:51:05  00:52:08
   62.7s     62.6s     63.7s     62.9s     62.9s     62.8s     62.6s
```

렌더 40회가 지나간 구간(62.9s)이 조용한 구간(62.6~63.7s)과 구분되지 않는다.

### 실측이 증명하지 않는 것

간격당 엔진 읽기가 **정확히** 1벌이라는 것은 이 실측이 보인 사실이 아니다.
프로세스 밖에서는 RPC 호출 수를 셀 수 없고, 여기서 관측한 것은 지연의 모양이다.
정확한 횟수는 `TestRedrawingTheLineDoesNotAskTheEngineAgainWithinTheInterval`과
`TestConcurrentRendersCostOneReading`이 세는 것이고, 실측은 그 캐시가 배포된
바이너리에 실제로 들어 있고 엔진 부하를 늘리지 않는다는 것을 확인한다.

화면에 그려진 값 자체(보유 종목의 보호선·관리 배지)는 계좌 정보라 이 기록에
넣지 않는다. 사람이 브라우저에서 직접 본다.

### a080 재개

a081이 land하고 실측까지 끝났으므로 a080의 `[blocked]`를 해제한다 (task 7.5).
남은 것은 a080 8장 — 코드 patch 복원, F2·F6·F7, base 재고정, AST 재생성, 그리고
두 번째 MODIFIED에 대한 Requirement-change 리뷰다.
