...
| `m.Quarantine` (a067) | active quarantine of this position's current generation, or nil | `ReadOnly.accountActiveQuarantines` | nil -> `Quarantined()` false, the pre-a067 rendering |
...
**a067 changes exactly one line** inside B2's body: `row.Quarantine = m.Quarantine`,
...
- The quarantine pointer is copied, not dereferenced: a nil stays nil and the
  row behaves exactly as it did before a067.
