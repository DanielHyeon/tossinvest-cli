...
- Base commit: `openspec/changes/a079-operator-can-lift-a-quarantine/base-commit.txt`
...
| `c.holdings.peek(now)` | 항상 사용 가능 | 콘솔 캐시 | 이름 없으면 빈 문자열 (a077) |
...
| `c.holdingNames(c.now())` | 종목명 (a077) | broker 호출 0 — 캐시 peek | AST |
| `c.policyAction` | 서명된 1회 select token | 순수 + HMAC | AST |
| `c.render` | 템플릿 렌더 | — | AST |
| **(a079)** 격리 목록 조회 | badge/action 재료 | 오류는 문구로 표시, 렌더 계속 | 신규 |
