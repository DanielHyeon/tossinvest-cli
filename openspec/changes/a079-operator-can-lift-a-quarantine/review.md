...
```text
- change id / task id: a079-operator-can-lift-a-quarantine / 8.1
...
- 원인이 a078 결함이었다면 이제 정상 전진으로 선택되고 판정이 이어진다.
...
2. 격리 **생성** 시점의 관측과 critical 알림 전달은 a074가 맡는다.
...
- `go test ./...` — 79 패키지 **6094건 green** (a079 신규 44건 포함).
...
- 격리 **생성** 시점의 관측과 critical 알림 전달은 a074.
- 해제 이력 화면 부재(I6), a078 테스트가 실제로 무엇을 덮고 있었는지 재확인(I4)은
  후속 후보다.
