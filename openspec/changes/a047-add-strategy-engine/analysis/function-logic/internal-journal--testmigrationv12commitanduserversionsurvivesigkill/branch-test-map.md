...
| B3 | closing the seeded old journal succeeds | same test | baseline | pass |
...
| B6 | each required v12/v13 artifact exists exactly once | same test artifact loop | baseline | pass |
| B7 | artifact query/count fails closed | same test assertion | baseline | pass |
| B8 | raw inspection handle closes cleanly | same test | baseline | pass |
| B9 | reopened legacy row counts are byte-preserving | same test | baseline | pass |
