...
- [ ] 3.1 Add an atomic additive migration for monetary bucket policies/snapshots, strategy keys, prospective/actual lane owners, reservations and events with explicit price/fee/FX provenance.
- [ ] 3.2 Update schema golden tests for migration atomicity, legacy unknown state and ErrSchemaTooNew without defaulting missing exposure, FX, sector or ownership.
- [ ] 3.3 Implement journal replay/read primitives that reconstruct owner, HELD/filled usage and digest, returning stable mismatches without automatic owner replacement or reservation deletion.
