...
- Inputs are the parameters in `TestTrackedFillOrders(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills_test.go:385 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/fills_test.go:392 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/journal/fills_test.go:397 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/journal/fills_test.go:400 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B5 | if | internal/journal/fills_test.go:403 | Preserve the explicit condition, early return, and fail-closed error behavior. |
