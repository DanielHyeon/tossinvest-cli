...
- Inputs are the parameters in `TestPollRejectsTrackedReadFromAnotherCanonicalScope(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/filldetect/detect_test.go:475 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/filldetect/detect_test.go:479 | Preserve the explicit condition, early return, and fail-closed error behavior. |
