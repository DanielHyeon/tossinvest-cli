...
- Inputs are the parameters represented by `TestStartupSweepsExpiredUnconsumedReservationsBeforeTrading(params=1, results=0)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/app/engine/engine_test.go:474 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/app/engine/engine_test.go:478 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/app/engine/engine_test.go:481 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
...
- `seedExpiredUnconsumedReservation`: errors and returned state remain governed by the function's explicit branches.
...
- `eng.Journal.LookupReservation`: errors and returned state remain governed by the function's explicit branches.
...
- `reservation.Held`: errors and returned state remain governed by the function's explicit branches.
