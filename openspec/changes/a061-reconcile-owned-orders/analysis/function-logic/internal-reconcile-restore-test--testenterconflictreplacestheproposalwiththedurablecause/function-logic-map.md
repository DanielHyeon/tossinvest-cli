...
- Inputs are the parameters in `TestEnterConflictReplacesTheProposalWithTheDurableCause(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:611 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/reconcile/restore_test.go:614 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/reconcile/restore_test.go:618 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/reconcile/restore_test.go:621 | Preserve the explicit condition, early return, and fail-closed error behavior. |
