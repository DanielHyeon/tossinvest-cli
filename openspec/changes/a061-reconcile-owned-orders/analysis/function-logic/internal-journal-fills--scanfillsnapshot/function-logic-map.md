...
- Inputs are the parameters represented by `scanFillSnapshot(params=1, results=2)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills.go:601 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/journal/fills.go:604 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |

## Calls and live bindings

- `row.Scan`: errors and returned state remain governed by the function's explicit branches.
