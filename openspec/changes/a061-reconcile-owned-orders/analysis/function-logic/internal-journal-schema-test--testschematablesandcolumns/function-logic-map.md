...
- Inputs are the parameters represented by `TestSchemaTablesAndColumns(params=1, results=0)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/schema_test.go:122 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | for | internal/journal/schema_test.go:126 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/journal/schema_test.go:128 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B4 | if | internal/journal/schema_test.go:133 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B5 | if | internal/journal/schema_test.go:137 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B6 | range | internal/journal/schema_test.go:249 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B7 | if | internal/journal/schema_test.go:252 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
...
- `tableColumns`: errors and returned state remain governed by the function's explicit branches.
- `sort.Strings`: errors and returned state remain governed by the function's explicit branches.
