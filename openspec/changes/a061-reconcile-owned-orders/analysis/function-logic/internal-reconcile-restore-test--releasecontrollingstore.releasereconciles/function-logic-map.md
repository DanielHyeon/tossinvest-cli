...
- Inputs are the parameters represented by `releaseControllingStore.ReleaseReconciles(params=2, results=2)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:105 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/reconcile/restore_test.go:108 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
