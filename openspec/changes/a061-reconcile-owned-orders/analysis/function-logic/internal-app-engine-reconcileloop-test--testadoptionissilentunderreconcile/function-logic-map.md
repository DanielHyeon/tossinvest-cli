...
- Inputs are the parameters in `TestAdoptionIsSilentUnderReconcile(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/app/engine/reconcileloop_test.go:359 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/app/engine/reconcileloop_test.go:366 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/app/engine/reconcileloop_test.go:370 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/app/engine/reconcileloop_test.go:374 | Preserve the explicit condition, early return, and fail-closed error behavior. |
...
- `h.alerts.count`: returned errors and state follow the mapped branches.
