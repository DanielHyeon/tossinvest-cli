...
- Inputs are the parameters in `enterReconcileScopeInTx(params=7, results=1)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/position_projection.go:389 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/position_projection.go:396 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/journal/position_projection.go:400 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/journal/position_projection.go:407 | Preserve the explicit condition, early return, and fail-closed error behavior. |
...
- `reconcileStateID`: returned errors and state follow the mapped branches.
- `nullableString`: returned errors and state follow the mapped branches.
