...
- Inputs are the parameters represented by `TestCollidingConfirmedOrderIdentityBlocksWithoutProjectingAPosition(params=1, results=0)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/position_projection_test.go:187 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/journal/position_projection_test.go:190 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/journal/position_projection_test.go:194 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B4 | if | internal/journal/position_projection_test.go:197 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
