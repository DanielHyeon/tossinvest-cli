...
- Inputs are the parameters in `hasReplaceSuccessor(params=4, results=2)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/position_projection.go:243 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/position_projection.go:248 | Preserve the explicit condition, early return, and fail-closed error behavior. |
