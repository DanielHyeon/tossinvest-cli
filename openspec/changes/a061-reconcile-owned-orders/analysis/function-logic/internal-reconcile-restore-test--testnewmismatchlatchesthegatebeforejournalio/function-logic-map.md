...
- Inputs are the parameters in `TestNewMismatchLatchesTheGateBeforeJournalIO(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:674 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/reconcile/restore_test.go:677 | Preserve the explicit condition, early return, and fail-closed error behavior. |
