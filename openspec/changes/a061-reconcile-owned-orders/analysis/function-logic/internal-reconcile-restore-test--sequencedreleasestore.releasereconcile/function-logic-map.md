...
- Inputs are the parameters in `sequencedReleaseStore.ReleaseReconcile(params=2, results=3)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:67 | Preserve the explicit condition, early return, and fail-closed error behavior. |

## Calls and live bindings

- `errors.New`: returned errors and state follow the mapped branches.
- `s.ReconcileStore.ReleaseReconcile`: returned errors and state follow the mapped branches.
