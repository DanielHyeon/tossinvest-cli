...
- Inputs are the parameters represented by `sequencedReleaseStore.ReleaseReconcile(params=2, results=3)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:67 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |

## Calls and live bindings

- `errors.New`: errors and returned state remain governed by the function's explicit branches.
- `s.ReconcileStore.ReleaseReconcile`: errors and returned state remain governed by the function's explicit branches.
