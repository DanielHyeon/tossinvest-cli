...
- Inputs are the parameters in `TestBlankPayloadIdFallsBackToTheTrackedId(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/filldetect/payload_test.go:143 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/filldetect/payload_test.go:147 | Preserve the explicit condition, early return, and fail-closed error behavior. |
