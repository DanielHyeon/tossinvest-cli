...
- Inputs are the parameters in `TestIncludeOnlyAdoptionRequiresAPriceReader(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/app/engine/reconcileloop_test.go:435 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/app/engine/reconcileloop_test.go:446 | Preserve the explicit condition, early return, and fail-closed error behavior. |
...
- `t.Cleanup`: returned errors and state follow the mapped branches.
- `j.Close`: returned errors and state follow the mapped branches.
- `engine.NewReconcileDriver`: returned errors and state follow the mapped branches.
