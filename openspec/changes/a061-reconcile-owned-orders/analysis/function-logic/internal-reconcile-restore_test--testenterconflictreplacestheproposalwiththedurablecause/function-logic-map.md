...
- Inputs are `TestEnterConflictReplacesTheProposalWithTheDurableCause(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:611 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/reconcile/restore_test.go:614 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/reconcile/restore_test.go:618 | Preserve the condition, error propagation, and fail-closed behavior. |
| B4 | if | internal/reconcile/restore_test.go:621 | Preserve the condition, error propagation, and fail-closed behavior. |
