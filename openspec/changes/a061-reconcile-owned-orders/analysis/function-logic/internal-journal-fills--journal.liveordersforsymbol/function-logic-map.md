...
- Inputs are the parameters represented by `Journal.LiveOrdersForSymbol(params=4, results=2)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills.go:1178 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | for | internal/journal/fills.go:1184 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/journal/fills.go:1186 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B4 | if | internal/journal/fills.go:1193 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B5 | range | internal/journal/fills.go:1197 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B6 | if | internal/journal/fills.go:1205 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
