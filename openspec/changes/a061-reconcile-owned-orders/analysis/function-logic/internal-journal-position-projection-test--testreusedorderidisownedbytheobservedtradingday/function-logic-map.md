...
- Inputs are the parameters represented by `TestReusedOrderIDIsOwnedByTheObservedTradingDay(params=1, results=0)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/position_projection_test.go:217 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/journal/position_projection_test.go:221 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/journal/position_projection_test.go:225 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B4 | if | internal/journal/position_projection_test.go:228 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
