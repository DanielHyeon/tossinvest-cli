...
- Inputs are the parameters in `TestReusedOrderIDIsOwnedByTheObservedTradingDay(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/position_projection_test.go:217 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/position_projection_test.go:221 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/journal/position_projection_test.go:225 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/journal/position_projection_test.go:228 | Preserve the explicit condition, early return, and fail-closed error behavior. |
