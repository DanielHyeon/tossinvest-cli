...
- Inputs are `TestPrunePreservesSpentNonceWhileItsReservationIsHeld(params=1, results=0)` parameters and receiver or package state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/nonce_test.go:263 | Preserve the condition, error propagation, and fail-closed behavior. |
| B2 | if | internal/journal/nonce_test.go:271 | Preserve the condition, error propagation, and fail-closed behavior. |
| B3 | if | internal/journal/nonce_test.go:274 | Preserve the condition, error propagation, and fail-closed behavior. |
...
- `spendNonce`: errors and state follow mapped branches.
- `fake.Advance`: errors and state follow mapped branches.
- `j.PruneSpentNonces`: errors and state follow mapped branches.
- `fake.Now`: errors and state follow mapped branches.
