...
- Inputs are the parameters represented by `TestTrackedFillOrdersRejectSnapshotIdentityMismatch(params=1, results=0)` and any receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills_test.go:437 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B2 | if | internal/journal/fills_test.go:442 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B3 | if | internal/journal/fills_test.go:446 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
| B4 | if | internal/journal/fills_test.go:449 | Preserve the explicit condition, early return, and fail-closed error behavior at this branch. |
