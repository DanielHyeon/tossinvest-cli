...
- Inputs are the parameters in `TestDetectionNeedsNoWTSSession(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/filldetect/detect_test.go:858 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/filldetect/detect_test.go:861 | Preserve the explicit condition, early return, and fail-closed error behavior. |
...
- `pollStart.Add`: returned errors and state follow the mapped branches.
- `newPager`: returned errors and state follow the mapped branches.
- `page`: returned errors and state follow the mapped branches.
- `filled.json`: returned errors and state follow the mapped branches.
- `newLedger`: returned errors and state follow the mapped branches.
