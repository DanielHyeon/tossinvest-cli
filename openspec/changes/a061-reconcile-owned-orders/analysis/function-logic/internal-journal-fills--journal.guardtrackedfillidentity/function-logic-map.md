...
- Inputs are the parameters in `Journal.guardTrackedFillIdentity(params=2, results=1)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills.go:1317 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/fills.go:1320 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/journal/fills.go:1325 | Preserve the explicit condition, early return, and fail-closed error behavior. |
