...
- Inputs are the parameters in `recordConfirmedLedgerOrder(params=3, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/filldetect/ledger_test.go:64 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/filldetect/ledger_test.go:67 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/filldetect/ledger_test.go:70 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/filldetect/ledger_test.go:73 | Preserve the explicit condition, early return, and fail-closed error behavior. |
