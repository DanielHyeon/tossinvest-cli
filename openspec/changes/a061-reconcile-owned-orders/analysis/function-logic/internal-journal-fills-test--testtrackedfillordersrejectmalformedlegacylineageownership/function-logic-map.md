...
- Inputs are the parameters in `TestTrackedFillOrdersRejectMalformedLegacyLineageOwnership(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/journal/fills_test.go:1111 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/journal/fills_test.go:1116 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/journal/fills_test.go:1126 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/journal/fills_test.go:1129 | Preserve the explicit condition, early return, and fail-closed error behavior. |
