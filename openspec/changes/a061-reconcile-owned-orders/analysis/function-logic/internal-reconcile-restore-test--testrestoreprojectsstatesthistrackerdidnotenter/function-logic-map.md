...
- Inputs are the parameters in `TestRestoreProjectsStatesThisTrackerDidNotEnter(params=1, results=0)` and receiver state.
...
| --- | --- | --- | --- |
| B1 | if | internal/reconcile/restore_test.go:321 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B2 | if | internal/reconcile/restore_test.go:331 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B3 | if | internal/reconcile/restore_test.go:336 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B4 | if | internal/reconcile/restore_test.go:339 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B5 | if | internal/reconcile/restore_test.go:346 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B6 | if | internal/reconcile/restore_test.go:350 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B7 | if | internal/reconcile/restore_test.go:353 | Preserve the explicit condition, early return, and fail-closed error behavior. |
| B8 | if | internal/reconcile/restore_test.go:356 | Preserve the explicit condition, early return, and fail-closed error behavior. |
