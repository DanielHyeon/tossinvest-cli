# Function Logic Map: `TestAnExpiredApprovalSendsNothing`
...
|---|---|---|---|---|
| B1 | `if n := h.broker.mutationCount(); n != 0 {` (internal/console/console_test.go:452, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
| B2 | `if !final.Summary.Halted {` (internal/console/console_test.go:455, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
