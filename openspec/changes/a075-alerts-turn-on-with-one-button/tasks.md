...
- [x] 9.3 `check_analysis.py --change a075-alerts-turn-on-with-one-button`
- [x] 9.4 `make sdd-sync` → `make sdd-check`
- [x] 9.5 PM story/registry/feature/generated
- [x] 9.6 `make gate CHANGE=a075-alerts-turn-on-with-one-button`

## 10. 배포 후 실측

- [x] 10.1 배포 후 콘솔에서 알림을 켜고 테스트 메시지가 실제로 도착하는지 실측한다.

      **확인 (2026-09-08, 이미지 `5c77491d93d4`).** 사용자가 끄기 → 켜기를 눌러
      켜기 경로를 이 빌드에서 실행했고, 이어서 [테스트 한 통 더]로 보낸 메시지가
      폰에 도착했다. 기록은 `issues.md` §배포 후 실측이다.

      이 실측이 덮는 것은 **콘솔 → ntfy**이며 **엔진 → ntfy**는 덮지 않는다 —
      카드 자신이 "엔진이 보내는 실제 알림은 다음 엔진 기동부터"라고 적고, 엔진은
      지금 0 루프다. 그 나머지 절반은 a074 task 11.1이다.
