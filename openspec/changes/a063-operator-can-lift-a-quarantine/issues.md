...
## I1 — 운영 원장의 활성 격리는 3건이고, 셋 다 a062 결함이다
...
차례로 첫 rung을 밟으며 같은 결함에 걸렸다. a062 조사에서 예측한 blast radius가
그대로 실현된 것이다.
...
a063 범위 밖이며 후속 change 후보다.

## I7 — 격리 생성 시점의 관측과 알림은 a064가 맡는다

a063은 이미 만들어진 격리를 다루는 change다. 격리가 **생기는** 순간이 로그에 없다는
사실과 critical 알림 publisher가 미배선이라는 사실은 a064의 범위다.
