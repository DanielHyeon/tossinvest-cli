...
**보고하는 코드가 없었다.** a074가 그 문장을 참으로 만든다.
...
a079 issues I4는 "조작한 판정을 `RecordExitJudgement`에 넣어 재격리를 보는 것은
불가능하다"고 기록했다. 여전히 맞다 — 저장된 snapshot을 손대면
`ValidateRecoveryDerivation`이나 output digest가 먼저 corrupt로 걸러낸다.

a074는 다른 각도를 찾았다. **저장된 snapshot은 그대로 두고 `exit_states.entry_price`
...
a074는 그 결정을 바꾸지 않았다. 다만 이제 사이클 실패가 **로그에는** 남으므로, 두절
...
실패한 사실까지 기록된다 — a074가 만들려던 경로 그대로다. 그리고 이것이 a075가
...
## task 11.1은 여전히 측정 불가이고, I7의 세 종목은 갈렸다 (2026-09-09 원장 실측)

11.1이 요구하는 것은 격리 **생성** 이벤트다. 8월에 관측한 `exit.judgement_refused`
다섯 건은 이미 격리된 포지션에 대한 판정 거부였고, 지금은 그것도 나오지 않는다 —
활성 격리 5건이 전부 CLOSED 포지션에 있고, 열린 포지션 셋(IONQ·RGTI·TSLA)은 하나도
격리되지 않아 판정이 정상적으로 돈다. `/positions`에 다섯 값이 전부 숫자로 나오는
것이 그 증거다(a077 issues.md).

**I7 정정.** "466100·IONQ·TSLA는 배포 후 콘솔에서 해제할 수 있다"는 실제로 절반이
실행됐고 절반은 못 하게 됐다.

| 종목 | 그 뒤 무슨 일이 있었나 |
|---|---|
| us IONQ | 2026-08-04T04:50:09Z 사람이 콘솔에서 해제(`HUMAN_REPAIR`). 지금은 새 세대 `instance_seq=7`로 열려 있고 격리 없음 |
| us TSLA | 2026-08-04T04:50:23Z 사람이 콘솔에서 해제(`HUMAN_REPAIR`). 지금 `state=OPEN`, 격리 없음 |
| kr 466100 | 해제되지 않았다. 포지션이 CLOSED가 되면서 화면 범위 밖으로 나갔다 |

해제 표면이 지금 도달 불가능한 이유는 a079 issues.md의 §"남은 창이 닫혔다"에 적었다.

새 격리가 **열린** 포지션에 자연히 생기기 전에는 11.1을 측정할 수 없다. 만들어 내려면
원장 대사를 어긋나게 해야 하므로 하지 않는다. task 11.1은 열어 둔다.
