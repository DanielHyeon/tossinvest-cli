...
Source: `internal/enginelock/enginelock.go` (296-324). AST 기준 branches **3** / returns 4.
...
|---|---|---|---|---|---|
| B1 | `:301` 첫 write 실패 | **yes** | `301.46,303.3` count=**1** | `TestAFailedHoldPublishesNothing` | count=0 (없던 커버리지) |
| B2 | `:311` `for {}` | yes | `311.7,312.11` count=**1** | `TestReleasingRemovesTheMarker` | count=1 |
| B3 | `:312` `select` 3-way | **부분** | 갈래표 참조 | 아래 | 같은 상태 |

| B3 갈래 | 근거 블록 (편집 후) | 지는 테스트 |
|---|---|---|
| `<-h.done` `:313` | `313.18,314.11` count=**1** | `TestReleasingRemovesTheMarker` |
| `<-ctx.Done()` `:315` | `315.22,316.11` count=**0** | 없음 (물려받은 공백) |
| `at := <-ticker.C` `:317` | `317.26,318.22` count=**0** | 없음 — **1분 ticker는 테스트가 닿을 수 없다** |
...
|---|---|---|---|
| `(*Held).Release` | `:217` | **83.3%** | `TestReleasingRemovesTheMarker` · `TestAFailedHoldPublishesNothing` · `TestReleaseIsNotUndoneByARefreshAlreadyInFlight` |
| `(*Held).Ready` | `:250` | **88.9%** | `TestReadyPublishesTheSignalOnlyWhenItIsCalled` · `TestReadyIsIdempotentAndKeepsTheFirstInstant` · `TestAFailedHoldPublishesNothing` |
| `(*Held).refresh` | `:277` | **85.7%** | `TestRefreshPreservesTheReadySignal` · `TestReleaseIsNotUndoneByARefreshAlreadyInFlight` |
| `write` | `:340` | **53.6%** | 별도 묶음 `internal-enginelock--write` |
| `Status.Ready` | `:408` | **100.0%** | `TestReadyRequiresBothALiveMarkerAndASignal` |
...
| 뮤테이션 | 가한 것 | 죽은 테스트 | 원복 |
|---|---|---|---|
| **(d)** | `refresh`가 쓰기 직전에 `ReadyAt = nil` | `TestRefreshPreservesTheReadySignal` · `TestTheMarkerIsNeverReadHalfWritten` | sha `9399c1d68e1d` 동일 |
| (g) 자체 | `Status.Ready`에서 `s.Running &&` 제거 | `TestReadyRequiresBothALiveMarkerAndASignal` | sha `9399c1d68e1d` 동일 |
| (i) 자체 | `Ready`의 멱등 early return 제거 | `TestReadyIsIdempotentAndKeepsTheFirstInstant` | sha `9399c1d68e1d` 동일 |
| **(l)** §3.9 | 원자 교체를 `os.WriteFile`로 되돌린다 (A2 F3) | `TestTheMarkerIsNeverReadHalfWritten` | sha `9399c1d68e1d` 동일 |
| **(m)** §3.9 | `Release`가 `live`를 끄지 않는다 (A2 F4) | `TestReleaseIsNotUndoneByARefreshAlreadyInFlight` · `TestReleaseWinsAgainstConcurrentRefreshes` | sha `9399c1d68e1d` 동일 |
...
- 커버리지: `go test ./internal/enginelock -count=1 -covermode=set -coverprofile` exit 0 ·
  **24건 통과** · 패키지 79.3% · `-race` 포함 통과 (패키지 수치가 내려간 것은 `write`의
