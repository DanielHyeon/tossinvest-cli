# Function Logic Map: `TestCorruptKRMarketSnapshotDoesNotInvalidateSealedUSMarket`

- Source: `internal/protectionreadiness/dispatch_test.go` (47-62)
...
|---|---|---|---|
| B1 | if at 56:2 | `if got := snapshot.Dispatch(krScope, readinessNow); got.Allowed \|\| got.Code != RefusalStateCorrupt {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
| B2 | if at 59:2 | `if got := snapshot.Dispatch(usScope, readinessNow); !got.Allowed \|\| got.Code != RefusalNone {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
