...
- Source: `internal/protection/readiness_adapter_test.go` (23-45)
...
- Extractor counts: AST branches 5 · returns 0 · calls 15
...
|---|---|---|---|
| B1 | if at 25:2 | `if _, err := NewReadinessAdapter(nil, "acct", "production"); err == nil {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
| B2 | if at 30:2 | `if err != nil {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
| B3 | if at 34:2 | `if refusal == nil \|\| refusal.Code != protectionreadiness.RefusalMissingEvidence \|\| checkpoint.Valid() {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
| B4 | if at 37:2 | `if provider.calls != 1 {` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
| B5 | if at 42:2 | `if _, refusal = adapter.Check(context.Background(), ReadinessRequest{Market: "kr", OrderType: "LIMIT", Quan...` | _test.go — 커버 계측 밖(분기 진입 여부 미측정) |
