...
### D4. 저장소 밖 autostart 스크립트는 좁은 패턴으로 남는다 (초기 구현 결정)
...
### D4a. 운영자 승인 후 스크립트를 은퇴시킨다 (2026-08-03 후속 처분)

구현·실측 완료 후 운영자가 만료된 1회성 설치물을 제거하도록 별도로 지시했다. 제거 전
프로세스, systemd unit, cron, shell rc, `console-launch.sh`에 **실행 또는 등록 참조가
없음**을 확인했다. 저장소에는 설계 역사와 기존 구현 근거를 설명하는 텍스트 참조가
남으므로 "저장소에 참조가 없다"고 표현하지 않는다.

제거 뒤 soak 프로세스 패턴의 계약 상대는 외부 스크립트가 아니라 D5의
`soakArgs` 생성 argv 하나다. 따라서 제거는 런타임 동작이나 패턴을 바꾸지 않고, 더는
존재하지 않는 외부 파일과의 암묵적 결합만 끝낸다.
...
| 설치된 스크립트가 두 번째 서베이를 시작 | 초기 구현 시에는 서로 다른 기록이라 경합이 없음을 확인했다. 이후 운영자 승인으로 만료된 설치물을 은퇴시켜 위험을 제거했다 (`issues.md` I3) |
