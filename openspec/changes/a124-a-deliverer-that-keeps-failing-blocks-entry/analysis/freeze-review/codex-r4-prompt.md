...
Change under review (read ALL of it first; the design is revision 4):
...
- openspec/changes/a124-a-deliverer-that-keeps-failing-blocks-entry/review.md sections §0, §0.2 and §0.3 (findings F1–F13, N1–N8, R1–R8 and
  the two decisions M1 = C, M2 = (ii) taken by the change manager). Treat all earlier findings as claims to verify,
  not as truth.
...
1. For each earlier finding F1–F13, N1–N8 and R1–R8: RESOLVED / PARTIAL / NOT RESOLVED, one line why.
2. New findings table: id | severity (P0 = unsafe or unimplementable as written, P1 = must fix before freeze, P2 = should
   ...
   formula and its numbers), and whether spec delta and tasks now cover what the design promises. In revision 4 pay
   special attention to the acknowledgement-generation fence in D7 (is a memory-only critical section really all that
   protection-path callers can now wait on; can the fence be defeated or cause a lost latch; is dropping a judgement when
   an acknowledgement intervened safe) and the per-row counter lifecycle in D8.
