...
|---|---|---|---|---|
| B1 | existing if branch at line 549 | only the branch's documented state transition | existing return/error contract | `TestRemoteConfigurationIsAllOrNothing` |
| B2 | existing if branch at line 553 | only the branch's documented state transition | existing return/error contract | `TestRemoteConfigurationIsAllOrNothing` |
| B3 | existing else branch at line 555 | only the branch's documented state transition | existing return/error contract | `TestRemoteConfigurationIsAllOrNothing` |
| B4 | existing if branch at line 558 | only the branch's documented state transition | existing return/error contract | `TestRemoteConfigurationIsAllOrNothing` |
