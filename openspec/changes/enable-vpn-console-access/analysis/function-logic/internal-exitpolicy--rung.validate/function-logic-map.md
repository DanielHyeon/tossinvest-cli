...
|---|---|---|---|---|
| B1 | existing if branch at line 112 | only the branch's documented state transition | existing return/error contract | `TestValidate` |
| B2 | existing if branch at line 115 | only the branch's documented state transition | existing return/error contract | `TestValidate` |
| B3 | existing if branch at line 119 | only the branch's documented state transition | existing return/error contract | `TestValidate` |
| B4 | existing if branch at line 122 | only the branch's documented state transition | existing return/error contract | `TestValidate` |
