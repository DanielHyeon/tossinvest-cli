...
- [ ] 2.1 `(position_id, adoption_generation)` lifecycle key를 갖는 additive override/generation schema와 repository transaction을 구현한다.
...
- [ ] 3.3 session+CSRF+version-bound opaque preview token console routes가 engine-owned commander만 호출하게 하고, 영향 종목/generation preview와 명확한 before/after/restart 설명·stale 412를 구현한다.
...
- [ ] 3.6 text/number/textarea/contenteditable·임의 symbol/reason 입력이 0개이고 hidden field는 CSRF와 opaque token뿐인 DOM/정적 테스트를 통과한다.
