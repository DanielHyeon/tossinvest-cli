# Function Logic Map: `fakeBroker.CancelOrder`
...
|---|---|---|---|---|
| B1 | `if f.cancelAlreadyProcessing > 0 {` (internal/verifylive/fake_broker_test.go:425, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
| B2 | `if !ok {` (internal/verifylive/fake_broker_test.go:430, if) | 아래 State mutations 참조 | 조건 불충족 시 조기 반환 또는 분기 | 이 함수 자체가 테스트다 |
