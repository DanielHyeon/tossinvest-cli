...
1. 버그 수정 시 **근본 원인** 파악 → 정석 수정 (증상 우회 금지)
2. 코드 수정 후 반드시 관련 테스트 실행 확인
3. 운영 반영 = 이미지 재빌드 + 컨테이너 재시작 (위 절차)
4. DB 상태 확인은 Postgres 직접 쿼리
5. 문서.코드.테스트 일치 → docs/WORKFLOW.md의 SDD 방법론을 반드시 따를것
6. 초등학생도 이해할 수 있게 한글 주석
7. 구현 후 반드시 gstack 리뷰 검증
8. 코드 구현 시 다하지도 않고 다했다고 거짓말 하지 않음
