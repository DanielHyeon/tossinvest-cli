...
func TestTokenGenerationFenceRunsAfterOldSoakExitBeforeSpawn(t *testing.T) {
	...
	soakSpawnDetached = func(binary, log string, args []string) error {
		if !prepared {
			t.Fatal("new soak spawned before the token-generation fence")
		}
		return spawn(binary, log, args)
	}

	if _, err := restartSoak(nil, record, func() error {
...
func TestTokenGenerationFenceFailureBlocksSoakSpawn(t *testing.T) {
	...
	_, err := restartSoak(nil, record, func() error { return errors.New("token cache busy") })
