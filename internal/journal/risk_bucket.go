...
func (j *Journal) CommitRiskBucketAdmission(ctx context.Context, plan RiskBucketAdmissionPlan) (RiskBucketAdmissionReceipt, error) {
	...
	snapshotSetDigest := riskBucketSnapshotSetDigest(plan.Snapshots)
	...
	for i, cap := range decision.Caps {
		...
		policyRecordDigest := riskBucketRecordDigest(struct {
			Key      riskbucket.BucketKey
		...
		snapshotRecordDigest := riskBucketRecordDigest(struct {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO risk_bucket_state_snapshots(snapshot_id,account_ref,market,symbol,prospective_generation,state_digest,event_sequence,created_at) VALUES(?,?,?,?,?,?,?,?)`, plan.TransactionID+":state:"+strconv.FormatInt(ownerSequence, 10), plan.Owner.Key.AccountID, string(plan.Owner.Key.Market), plan.Owner.Key.Symbol, plan.Owner.Key.ProspectiveGeneration, stateDigest, ownerSequence, canonicalRiskTime(plan.CreatedAt))
	if err != nil {
		return RiskBucketAdmissionReceipt{}, err
	}
...
func riskBucketSnapshotSetDigest(refs []RiskBucketSnapshotReference) string {
	raw, _ := json.Marshal(refs)
	sum := sha256.Sum256(raw)
	return hex.EncodeToString(sum[:])
}

func riskBucketRecordDigest(value any) string {
...
func loadRiskBucketState(ctx context.Context, q riskBucketQueryer, key riskbucket.OwnerKey) (RiskBucketState, string, error) {
	...
	return RiskBucketState{Owner: riskbucket.OwnerClaim{Key: key, LaneID: lane, CampaignID: campaign}, QFinal: qTotal, Reservations: reservations, Usage: usage}, digest, nil
}
