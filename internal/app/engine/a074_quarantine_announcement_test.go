...
// a074 task 3: the moment protection stops is an event of its own.
//
...
func TestAnAmbiguousRecoveryQuarantineIsAnnouncedInTheSameCycle(t *testing.T) {
	...
	// forging a snapshot — which a079 (issues I4) established is unreachable.
	db := openLedger(t, h)
...
func TestANewQuarantineVersionIsAnnouncedAgain(t *testing.T) {
	...
	// a079's release, then the same unresolved cause quarantines it again under a
