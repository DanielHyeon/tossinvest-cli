...
// a079 task 5: the three quarantine routes on the engine's loopback endpoint.
//
// The claim that matters most here is the negative one: a command service
// without the capability serves exactly the route set it served before a079.
...
func TestAServiceWithoutQuarantineCapabilityRegistersNoNewRoutes(t *testing.T) {
	// A command service that predates a079: the mux must be exactly what it was.
	dir := privateEngineTestDir(t)
	...
	for _, path := range []string{"/v1/quarantines", "/v1/quarantine/preview", "/v1/quarantine/release"} {
		...
		if resp.StatusCode != http.StatusNotFound {
			t.Errorf("%s on a pre-a079 service = %d, want 404", path, resp.StatusCode)
		}
	}
}
