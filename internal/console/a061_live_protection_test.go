...
// a061: the protection line is a state, not a measurement.
//
...
func seedUnchangedJudgement(t *testing.T, h *livePositionHarness, observedAt string) exitpolicy.ExitLineSnapshot {
	...
	line, recovery := ratchetViewSnapshot(t, "pos-managed", 1, "10", "obs-a061",
...
// TestTheProtectionLineSurvivesAJudgementThatChangedNothing is a061 tasks 2.1 and
...
//
// This is the safety precondition of the whole change. Before a061 the console
...
func TestAReleasedGenerationsQuarantineDoesNotCloseTheCurrentOne(t *testing.T) {
	...
	line, recovery := ratchetViewSnapshot(t, "pos-managed", 2, "10", "obs-a061-gen2",
		"70500", "70000", "68000", "0", exitpolicy.LevelNone)
...
// had before a061 — including its wording. This is the regression guard for
...
func TestAConsoleWithNoEngineMarkerKeepsTheObservationAgeBound(t *testing.T) {
	...
	seedJournal(t, h.journal)
	line, recovery := ratchetViewSnapshot(t, "pos-managed", 1, "10", "obs-a061-unwired",
		"70500", "70000", "68000", "0", exitpolicy.LevelNone)
	writeViewSnapshot(t, h.journal, line, recovery, "2026-07-26T19:00:00Z")
	h.authenticate(t)
	...
	for _, want := range []string{"오래된 평가", "평가 시각이 표시 허용 범위를 지났다"} {
		if !strings.Contains(page, want) {
			t.Errorf("an unwired console lost the pre-a061 wording %q", want)
		}
	}
...
func TestExitLinesStayClosedWhenTheEvidenceCannotBeTrusted(t *testing.T) {
	for _, c := range []struct{ name, observedAt, reason string }{
		...
		// gets. The case is here so a061 cannot quietly make a corrupt snapshot
