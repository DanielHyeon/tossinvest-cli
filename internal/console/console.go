...
type Options struct {
	...
	// Notifications is the console's write surface for where critical alerts go
	// (change a065). Its Enable takes no argument, so this screen chooses no
	// value that lands in the file: the channel is generated by the seam and the
	// transport's token has no console surface at all. Nil renders the alert card
	// read-only with the reason 미배선.
...
func (c *Console) routes() http.Handler {
	...
	// Critical alert delivery (change a065). Three acts on one card: create the
	// channel and turn delivery on, send one test message, turn delivery off. The
	// paths say what they are for the reason /settings/gate does — a route that
	// configures where account events are sent should not be readable only by
	// following the handler.
	mux.HandleFunc("/settings/notifications/on",
		c.session0(c.mutating(c.handleSettingsNotificationsOn)))
	mux.HandleFunc("/settings/notifications/test",
		c.session0(c.mutating(c.handleSettingsNotificationsTest)))
	mux.HandleFunc("/settings/notifications/off",
		c.session0(c.mutating(c.handleSettingsNotificationsOff)))
	...
	// Change a069's quarantine release, under the same wrapping as the policy
