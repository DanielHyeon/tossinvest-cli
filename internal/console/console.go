...
type Options struct {
	...
	// (change a075). Its Enable takes no argument, so this screen chooses no
...
func (c *Console) routes() http.Handler {
	...
	// Critical alert delivery (change a075). Three acts on one card: create the
	...
	// Change a079's quarantine release, under the same wrapping as the policy
